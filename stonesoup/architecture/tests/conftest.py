...
from ..node import FusionNode, Node, RepeaterNode, SensorFusionNode, SensorNode
...
def nodes():
    E = np.array(
        [
            [0.8, 0.1],  # P(small | bike), P(small | car)
            [0.19, 0.3],  # P(medium | bike), P(medium | car)
            [0.01, 0.6],
        ]
    )  # P(large | bike), P(large | car)

    model = MarkovianMeasurementModel(
        emission_matrix=E, measurement_categories=["small", "medium", "large"]
    )
...
def data_pieces(times, nodes):
    data_piece_a = DataPiece(
        node=nodes["a"], originator=nodes["a"], data=Track([]), time_arrived=times["a"]
    )
    data_piece_b = DataPiece(
        node=nodes["a"], originator=nodes["b"], data=Track([]), time_arrived=times["b"]
    )
    data_piece_fail = DataPiece(
        node=nodes["a"],
        originator=nodes["b"],
        data="Not a compatible data type",
        time_arrived=times["b"],
    )
    data_piece_hyp = DataPiece(
        node=nodes["a"], originator=nodes["b"], data=Hypothesis(), time_arrived=times["b"]
    )
...
def track_tracker(initiator, deleter, fusion_queue, data_associator, detection_track_updater):
    track_tracker = MultiTargetTracker(
        initiator,
        deleter,
        Tracks2GaussianDetectionFeeder(fusion_queue),
        data_associator,
...
def radar_nodes(tracker, track_tracker, radar_sensors, fusion_queue):
    ...
    node_I = SensorFusionNode(
        sensor=sensor_set[5], tracker=node_I_tracker, fusion_queue=node_I_tracker.detector
    )
...
def edge_lists(nodes, radar_nodes):
    hierarchical_edges = Edges(
        [
            Edge((nodes["s2"], nodes["s1"])),
            Edge((nodes["s3"], nodes["s1"])),
            Edge((nodes["s4"], nodes["s2"])),
            Edge((nodes["s5"], nodes["s2"])),
            Edge((nodes["s6"], nodes["s3"])),
            Edge((nodes["s7"], nodes["s6"])),
        ]
    )
    ...
    centralised_edges = Edges(
        [
            ...
            Edge((nodes["s6"], nodes["s3"])),
            Edge((nodes["s7"], nodes["s6"])),
            Edge((nodes["s7"], nodes["s5"])),
            Edge((nodes["s5"], nodes["s3"])),
        ]
    )
    ...
    linear_edges = Edges(
        [
            Edge((nodes["s1"], nodes["s2"])),
            Edge((nodes["s2"], nodes["s3"])),
    ...
    decentralised_edges = Edges(
        [
            ...
            Edge((nodes["s3"], nodes["s1"])),
            Edge((nodes["s3"], nodes["s4"])),
            Edge((nodes["s3"], nodes["s5"])),
            Edge((nodes["s5"], nodes["s4"])),
        ]
    )

    disconnected_edges = Edges(
    ...
    k4_edges = Edges(
        [
            Edge((nodes["s1"], nodes["s2"])),
            Edge((nodes["s1"], nodes["s3"])),
            Edge((nodes["s1"], nodes["s4"])),
            Edge((nodes["s2"], nodes["s3"])),
            Edge((nodes["s2"], nodes["s4"])),
            Edge((nodes["s3"], nodes["s4"])),
        ]
    )
    ...
    radar_edges = Edges(
        [
            Edge((radar_nodes["a"], radar_nodes["c"])),
