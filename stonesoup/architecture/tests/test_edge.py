...
def test_update_messages():
    ...
    _, edge.messages_held = _dict_set(edge.messages_held, message, "pending", message.arrival_time)
...
def test_edges(edges, nodes):
    ...
    assert list(empty_edges) == []
    empty_edges.add(Edge((nodes["a"], nodes["b"])))
    assert list(empty_edges) == [Edge((nodes["a"], nodes["b"]))]
...
def test_message(edges, data_pieces, times):
    ...
    edge = edges["a"]
    message = Message(
        edge=edge, time_pertaining=times["a"], time_sent=times["b"], data_piece=data_pieces["a"]
    )
...
def test_message_destinations(times, radar_nodes):
    ...
    message1 = Message(
        ...
        start_time,
        DataPiece(node1, node1, Track([]), datetime.datetime(2016, 1, 2, 3, 4, 5)),
    )
    ...
    data_held = []
    for time in node2.data_held["unfused"]:
