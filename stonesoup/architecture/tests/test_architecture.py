...
from .. import InformationArchitecture, NetworkArchitecture, NonPropagatingArchitecture
from ..edge import DataPiece, Edge, Edges, FusionQueue, Message
...
def test_is_hierarchical(edge_lists):
    ...
    disconnected_architecture = InformationArchitecture(
        edges=disconnected_edges, force_connected=False
    )
...
def test_is_centralised(edge_lists):
    ...
    disconnected_loop_architecture = InformationArchitecture(
        edges=disconnected_loop_edges, force_connected=False
    )
...
def test_all_nodes(nodes, edge_lists):
    ...
    hierarchical_architecture = InformationArchitecture(edges=hierarchical_edges)
    assert hierarchical_architecture.all_nodes == {
        nodes["s1"],
        nodes["s2"],
        nodes["s3"],
        nodes["s4"],
    ...
    centralised_architecture = InformationArchitecture(edges=centralised_edges)
    assert centralised_architecture.all_nodes == {
        nodes["s1"],
        nodes["s2"],
        nodes["s3"],
        nodes["s4"],
    ...
    decentralised_architecture = InformationArchitecture(edges=decentralised_edges)
    assert decentralised_architecture.all_nodes == {
        nodes["s1"],
        nodes["s2"],
        nodes["s3"],
        nodes["s4"],
    ...
    linear_architecture = InformationArchitecture(edges=linear_edges)
    assert linear_architecture.all_nodes == {
        nodes["s1"],
        nodes["s2"],
        nodes["s3"],
        nodes["s4"],
    ...
    assert disconnected_architecture.all_nodes == {
        nodes["s1"],
        nodes["s2"],
        nodes["s3"],
        nodes["s4"],
    ...
    circular_architecture = InformationArchitecture(edges=circular_edges)
    assert circular_architecture.all_nodes == {
        nodes["s1"],
        nodes["s2"],
        nodes["s3"],
        nodes["s4"],
...
def test_information_arch_measure_no_noise(edge_lists, ground_truths, times):
    ...
    start_time = times["start"]
    network = InformationArchitecture(edges=edges)
    all_detections = network.measure(
        ground_truths=ground_truths, current_time=start_time, noise=False
    )
...
def test_fully_propagated(edge_lists, times, ground_truths):
    ...
    for node in network.sensor_nodes:
        # Check that each sensor node has data held for the detection of all 3 targets
        for key in node.data_held["created"]:
...
def test_network_arch(radar_sensors, ground_truths, tracker, track_tracker, times):
    ...
    node_C = FusionNode(
        tracker=node_C_tracker,
        fusion_queue=node_C_tracker.detector,
        latency=0,
        label="FusionNode C",
    )
    ...
    network_arch = NetworkArchitecture(
        edges=Edges(
            [
                Edge((node_A, repeaternode1), edge_latency=0.5),
...
def test_network_arch_instantiation_methods(radar_nodes, times):
    ...
    net_arch2 = NetworkArchitecture(edges=network_edges, information_architecture_edges=info_edges)
...
def test_net_arch_fully_propagated(generator_params, ground_truths):
    ...
    edge = {
        ...
        if isinstance(edge.nodes[0], RepeaterNode) and isinstance(edge.nodes[1], FusionNode)
    }.pop()
    ...
    message = Message(
        ...
        DataPiece(
            ...
            Track(
                [
                    GaussianState(
                        [1, 2, 3, 4], np.diag([1, 1, 1, 1]), datetime.datetime(2016, 1, 2, 3, 4, 5)
                    )
                ]
            ),
            datetime.datetime(2016, 1, 2, 3, 4, 5),
