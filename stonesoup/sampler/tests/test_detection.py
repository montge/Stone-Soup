...
from ..detection import GaussianDetectionParticleSampler, SwitchingDetectionSampler
...
@pytest.mark.parametrize(
    ...
    [
        (
            LinearGaussian(
                ndim_state=2, mapping=(0, 1), noise_covar=np.eye(2)
            ),  # measurement_model
        ...
        (
            LinearGaussian(
                ndim_state=4, mapping=(0, 3), noise_covar=np.eye(2)
            ),  # measurement_model
            [
                np.array([10, 0, 10, 0]),
                np.array([20, 0, 20, 0]),
        ...
        (
            CartesianToBearingRange(
                ndim_state=4, mapping=(0, 2), noise_covar=np.diag([0.01, 1])
            ),  # measurement_model
            [
                np.array([10, 0, 10, 0]),
                np.array([20, 0, 20, 0]),
    ...
    ids=[
        "linear_1_detection",
        "linear_3_detections",
        "linear_3_partial_state_detections",
...
def test_gaussian_detection_sampler(measurement_model, states, nsamples):
    ...
    for ii in range(3):
        detections |= {
            Detection(
                measurement_model.function(State(states[ii])),
