...
class SingleTargetGroundTruthSimulator(GroundTruthSimulator):
    ...
    def groundtruth_paths_gen(self):
        ...
        gttrack = GroundTruthPath(
            [
                GroundTruthState(
                    self.initial_state.state_vector, timestamp=time, metadata={"index": self.index}
                )
            ]
        )
        yield time, {gttrack}
        ...
        for _ in range(self.number_steps - 1):
            ...
            gttrack.append(
                GroundTruthState(
                    trans_state_vector, timestamp=time, metadata={"index": self.index}
                )
            )
            yield time, {gttrack}
...
class SwitchOneTargetGroundTruthSimulator(SingleTargetGroundTruthSimulator):
    ...
    model_probs: np.ndarray = Property(
        doc="A matrix of probabilities.\
    ...
    seed: int | None = Property(
        default=None, doc="Seed for random number generation." " Default None"
    )
    ...
    def transition_model(self):
        self.index = self.random_state.choice(
            range(0, len(self.transition_models)), p=self.model_probs[self.index]
        )
...
class MultiTargetGroundTruthSimulator(SingleTargetGroundTruthSimulator):
    ...
    birth_rate: float = Property(
        default=1.0,
        doc="Rate at which tracks are born. Expected number of occurrences (λ) in "
    ...
    def _new_target(self, time, random_state, state_vector=None):
        if state_vector is not None:
            ...
            vector = (
                ...
                + self.initial_state.covar @ random_state.randn(self.initial_state.ndim, 1)
            )

        gttrack = GroundTruthPath()
        gttrack.append(
            GroundTruthState(state_vector=vector, timestamp=time, metadata={"index": self.index})
        )
        return gttrack
...
class SwitchMultiTargetGroundTruthSimulator(MultiTargetGroundTruthSimulator):
    ...
    def transition_model(self, random_state=None):
        random_state = random_state if random_state is not None else self.random_state
        self.index = random_state.choice(
            range(0, len(self.transition_models)), p=self.model_probs[self.index]
        )
        return self.transition_models[self.index]
...
class SimpleDetectionSimulator(DetectionSimulator):
    ...
    def __in_state_space(self, detection):
        ...
        for dim in range(self.meas_range.ndim):
            if (
                not self.meas_range[dim][0]
                <= detection.state_vector[dim]
