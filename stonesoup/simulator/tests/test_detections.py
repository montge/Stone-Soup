...
def test_simple_detection_simulator(transition_model1, measurement_model, timestep):
    initial_state = State(np.array([[0], [0], [0], [0]]), timestamp=datetime.datetime.now())
    groundtruth = SingleTargetGroundTruthSimulator(transition_model1, initial_state, timestep)
    ...
    for (_, detections1), (_, detections2) in zip(
        simulate_detections1, simulate_detections2, strict=False
    ):
...
def test_switch_detection_simulator(
    ...
    detector = SwitchDetectionSimulator(
        groundtruth, measurement_model, meas_range, clutter_rate=3, detection_probabilities=[0, 1]
    )

    # Create reference detector with 1 detection probability.
    test_detector = SimpleDetectionSimulator(
        groundtruth, measurement_model, meas_range, clutter_rate=3, detection_probability=1
    )
    ...
    assert len(total_detections - clutter_detections) < len(
        test_detections - test_clutter_detections
    )
