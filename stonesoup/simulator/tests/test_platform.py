...
def build_platform(sensors, x_velocity):
    state = State(
        state_vector=[[0], [x_velocity], [0], [0]], timestamp=datetime.datetime(2020, 4, 1)
    )
...
def test_platform_detection_simulator(sensor_model1, sensor_model2, transition_model1):

    platform1 = build_platform([sensor_model1], 1)
    platform2 = build_platform([sensor_model2], 2)

    ground_truth = (
        (datetime.datetime(2020, 4, 1) + datetime.timedelta(seconds=i), set()) for i in range(10)
    )
    detector = PlatformDetectionSimulator(ground_truth, [platform1, platform2])

    for n, (_time, detections) in enumerate(detector):
        ...
        for detection in detections:
            if n % 2 == 0:
                ...
                assert detection.state_vector[0, 0] == platform2.state.state_vector[0, 0]
            else:
                # platform2 detects platform1.
                assert detection.state_vector[0, 0] == platform1.state.state_vector[0, 0]
        if n > 10:
            break


def test_platform_ground_truth_detection_simulator(
    sensor_model1, sensor_model2, transition_model1
):

    platform = build_platform([sensor_model1, sensor_model2], 1)

    initial_state = State(np.array([[0], [0], [0], [0]]), timestamp=datetime.datetime(2020, 4, 1))
    ground_truth = SingleTargetGroundTruthSimulator(
        transition_model1, initial_state, number_steps=10
    )
...
def test_detection_simulator(sensor_model1, sensor_model2, transition_model1):
