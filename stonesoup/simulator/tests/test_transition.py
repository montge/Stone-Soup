...
from ..transition import ConstantJerkSimulator, create_smooth_transition_models
...
def test_transitions_list(coords, turn_rate):
    ...
    transition_models, transition_times = create_smooth_transition_models(
        initial_state=target,
        x_coords=x_coords,
        y_coords=y_coords,
        times=times,
    ...
    for i in range(len(transition_models)):
        target.state_vector = transition_models[i].function(
            state=target, time_interval=transition_times[i]
        )
...
def test_constant_jerk():
    ...
    for _ in range(5):
        ...
        final_state = State(100 * np.random.rand(ndim_state, 1), start + timedelta(minutes=1))
        if ndim_state > 4:
    ...
    ndim_state = np.random.randint(4, 6)
    states = [
        State(100 * np.random.rand(ndim_state, 1), start + timedelta(minutes=i)) for i in range(5)
    ]
    if ndim_state > 4:
    ...
    transition_models, transition_times = ConstantJerkSimulator.create_models(
        states, position_mapping
    )

    assert len(transition_models) == len(transition_times) == 4  # one less than len(states)
    for prev_state, next_state, model, duration in zip(
        states[:-1], states[1:], transition_models, transition_times, strict=False
    ):
    ...
    for i, (model, duration) in enumerate(zip(transition_models, transition_times, strict=False)):
