...
from stonesoup.platform import (
    FixedPlatform,
    MovingPlatform,
...
def test_base():
    ...
    platform = MovingPlatform(
        states=platform_state2d, transition_model=model_2d, position_mapping=[0, 2]
    )
    platform.move(new_timestamp)
    ...
    model_3d = CombinedLinearGaussianTransitionModel([model_1d, model_1d, model_1d])

    # Define a 3d platform with constant velocity motion and test motion
    platform_state = State(np.array([[2], [1], [2], [1], [0], [1]]), timestamp)
    platform = MovingPlatform(
        states=platform_state, transition_model=model_3d, position_mapping=[0, 2, 4]
    )
    platform.move(new_timestamp)
...
def test_add_sensor(class_):
    ...
    sensor = DummySensor()
    platform = class_(states=platform_state, position_mapping=[0, 2, 4], **platform_args)
...
def test_velocity_properties(velocity_mapping):
    ...
    platform_state = State(np.array([[2], [1], [2], [1], [0], [1]]), timestamp)
    platform = MovingPlatform(
        states=platform_state, transition_model=None, position_mapping=[0, 2, 4]
    )
    assert platform.is_moving
    assert np.array_equal(platform.velocity, StateVector([1, 1, 1]))
    old_position = platform.position
...
def test_orientation_dimensionality_error():
    platform_state = State(
        StateVector([2, 1, 1, 1, 2, 0, 1, 0]), timestamp=datetime.datetime.now()
    )

    platform = MovingPlatform(
        states=platform_state, position_mapping=[0, 1, 2, 3], transition_model=None
    )
...
def test_moving_with_no_initial_timestamp():
    ...
    platform = MovingPlatform(
        states=platform_state, position_mapping=[0, 2, 4], transition_model=None
    )
...
orientation_tests_3d = [
    ...
    (StateVector([0, 1, 0, 1, 0, 1]), StateVector([0, np.arctan(1 / np.sqrt(2)), np.pi / 4])),
...
orientation_tests_2d = [
    (StateVector([0, 1, 0, 0]), StateVector([0, 0, 0])),
...
def test_platform_orientation_2d(state, orientation):
    ...
    platform_state = State(state, timestamp)
    platform = MovingPlatform(
        states=platform_state, transition_model=model_2d, position_mapping=[0, 2]
    )
    assert np.allclose(platform.orientation, orientation)
    # moving with a constant velocity model should not change the orientation
    platform.move(new_timestamp)
...
def test_setting_position():
    ...
    platform = MovingPlatform(
        states=platform_state, transition_model=model_3d, position_mapping=[0, 1, 2]
    )
    with pytest.raises(AttributeError):
    ...
    platform_state = State(np.array([[2], [2], [0]]), timestamp)
    platform = MovingPlatform(
        states=platform_state, transition_model=None, position_mapping=[0, 1, 2]
    )
    ...
    platform_state = State(np.array([[2], [1], [2], [-1], [2], [0]]), timestamp)
    platform = MovingPlatform(
        states=platform_state, transition_model=model_3d, position_mapping=[0, 2, 4]
    )
    with pytest.raises(AttributeError):
...
def test_setting_orientation():
    ...
    platform = FixedPlatform(
        states=platform_state, position_mapping=[0, 1, 2], orientation=platform_orientation
    )
    ...
    platform_orientation = StateVector([0, 0, 0])
    platform = FixedPlatform(
        states=platform_state, position_mapping=[0, 2, 4], orientation=platform_orientation
    )
    assert np.array_equal(platform.orientation, StateVector([0, 0, 0]))
    platform.orientation = StateVector([0, 1, 0])
    assert np.array_equal(platform.orientation, StateVector([0, 1, 0]))
...
def test_mapping_types(mapping_type):
    ...
    platform_state = State(np.array([[2], [1], [2], [-1], [2], [0]]), timestamp)
    platform = MovingPlatform(
        states=platform_state, transition_model=None, position_mapping=mapping_type([0, 2, 4])
    )
    assert np.array_equal(platform.position, StateVector([2, 2, 2]))
...
@pytest.mark.parametrize(
    "first_state, second_state, expected_measurement", position_measurement_sets
)
def test_range_and_angles_to_other(first_state, second_state, expected_measurement):
    ...
    timestamp = datetime.datetime.now()
    platform1 = MovingPlatform(
        states=State(first_state, timestamp=timestamp),
        position_mapping=(0, 2, 4),
        transition_model=None,
    )
    platform2 = MovingPlatform(
        states=State(second_state, timestamp=timestamp),
        position_mapping=(0, 2, 4),
        transition_model=None,
    )
...
def test_ground_truth_path_story():
    ...
    platform_gtp.states.append(
        State(np.array([[10], [9], [8], [7], [6], [5]]), timestamp + datetime.timedelta(seconds=2))
    )
...
def test_path(states):
    platform = PathBasedPlatform(
        path=gt, states=states, position_mapping=[0], transition_model=None
    )
    ...
    with pytest.raises(IndexError, match="timestamp not found in states"):
        gt[platform.states[-1].timestamp]


def test_path_no_states_no_tm():
    platform = PathBasedPlatform(path=gt, position_mapping=[0])
    platform.move(timestamp=start_time + datetime.timedelta(seconds=1))
    assert len(platform.states) == 2
    assert np.allclose(platform.states[-1].state_vector, GroundTruthState([-0.5, 1]).state_vector)
...
def test_obstacle(shape_data, position1, orientation1, position2, orientation2, simplices):
    ...
    if np.any(orientation1 != StateVector([[0], [0], [0]])) and simplices:
        ...
        test_obstacle = Obstacle(
            states=State(position1), orientation=orientation1, position_mapping=(0, 1), shape=shape
        )
    elif simplices:
        shape = Shape(
            shape_data=shape_data,
        ...
        test_obstacle = Obstacle(states=State(position1), position_mapping=(0, 1), shape=shape)
        ...
        with pytest.raises(
            ValueError,
            match="The 'Obstacle' platform type requires "
            "that property 'shape' is defined. Currently 'None'",
        ):
            test_obstacle = Obstacle(
                states=State(position1),
    ...
    rot_m = (
        ...
        @ np.array(
            [
                [np.cos(orientation1[1, 0]), 0, np.sin(orientation1[1, 0])],
                [0, 1, 0],
        ...
        @ np.array(
            [
                [np.cos(-orientation1[2, 0]), -np.sin(-orientation1[2, 0]), 0],
    ...
    if not simplices:
        simplices = np.roll(
            np.linspace(0, shape_data.shape[1] - 1, shape_data.shape[1]), -1
        ).astype(int)

    true_vertices = rot_m[:2, :2] @ test_obstacle.shape.shape_data[:, simplices] + position1

    edge_index = np.roll(np.linspace(0, len(simplices) - 1, len(simplices)), 1).astype(int)
    true_relative_edges = np.array(
        [
            true_vertices[0, :] - true_vertices[0, edge_index],
    ...
    if orientation2 is not None:
        rot_m = (
            ...
            @ np.array(
                [
                    [np.cos(orientation2[1, 0]), 0, np.sin(orientation2[1, 0])],
                    [0, 1, 0],
            ...
            @ np.array(
                [
                    [np.cos(-orientation2[2, 0]), -np.sin(-orientation2[2, 0]), 0],
    ...
    true_relative_edges2 = np.array(
        [
            true_vertices2[0, :] - true_vertices2[0, edge_index],
            true_vertices2[1, :] - true_vertices2[1, edge_index],
        ]
    )
...
@pytest.mark.parametrize(
    ...
    [
        (StateVector([3, 4]), None, None),  # position2  # orientation2  # mapping2
        (
            StateVector([3, 4]),  # position2
            StateVector([[0], [0], [np.radians(22.2)]]),  # orientation2
            None,
        ),  # mapping2
        (StateVector([3, 4, 5]), None, (1, 2)),  # position2  # orientation2  # mapping2
        (
    ...
    ids=[
        "from_obs_position_change",
        "from_obs_orientation_change",
        "from_obs_mapping_change",
...
def test_from_obstacle(position2, orientation2, mapping2):
    ...
    initial_obstacle = Obstacle(shape=shape, states=State(position), position_mapping=mapping)
    ...
    if orientation2 is not None and mapping2 is not None:
        sub_obstacle = Obstacle.from_obstacle(
            initial_obstacle,
        ...
        sub_obstacle = Obstacle.from_obstacle(
            initial_obstacle, states=State(position2), orientation=orientation2
        )
        rot_orient = orientation2
    elif mapping2 is not None:
        sub_obstacle = Obstacle.from_obstacle(
            initial_obstacle, states=State(position2), position_mapping=mapping2
        )
        rot_orient = initial_obstacle.orientation
    else:
        sub_obstacle = Obstacle.from_obstacle(initial_obstacle, states=State(position2))
        rot_orient = initial_obstacle.orientation
    ...
    rot_m = (
        ...
        @ np.array(
            [
                [np.cos(rot_orient[1, 0]), 0, np.sin(rot_orient[1, 0])],
                [0, 1, 0],
        ...
        @ np.array(
            [
                [np.cos(-rot_orient[2, 0]), -np.sin(-rot_orient[2, 0]), 0],
    ...
    true_vertices = (
        rot_m[:2, :2] @ shape_data[:, initial_obstacle.shape.simplices]
        + position2[mapping2 if mapping2 is not None else mapping, :]
    )
    edge_index = np.roll(
        np.linspace(
            0, len(initial_obstacle.shape.simplices) - 1, len(initial_obstacle.shape.simplices)
        ),
        1,
    ).astype(int)
    true_relative_edges = np.array(
