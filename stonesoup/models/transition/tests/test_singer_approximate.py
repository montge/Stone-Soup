...
def test_singer_approximate(singer_model_params, sign):  # noqa: F811
    ...
    if dim == 1:
        model_obj = SingerApproximate(
            noise_diff_coeff=noise_diff_coeffs[0], damping_coeff=damping_coeffs[0]
        )
    else:
        model_list = [
            SingerApproximate(
                noise_diff_coeff=noise_diff_coeffs[i], damping_coeff=damping_coeffs[i]
            )
    ...
    for i in range(0, dim):
        ...
        covar_list.append(
            np.array(
                [
                    [abs(timediff) ** 5 / 20, abs(timediff) ** 4 / 8, abs(timediff) ** 3 / 6],
                    [abs(timediff) ** 4 / 8, abs(timediff) ** 3 / 3, abs(timediff) ** 2 / 2],
                    [abs(timediff) ** 3 / 6, abs(timediff) ** 2 / 2, abs(timediff)],
                ]
            )
            * noise_diff_coeff
        )
    ...
    assert np.allclose(
        Q, model_obj.covar(timestamp=new_timestamp, time_interval=time_interval), rtol=1e-10
    )
