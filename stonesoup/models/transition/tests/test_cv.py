...
def test_cvmodel(sign):
    ...
    Q = (
        np.array(
            [
                [abs(timediff) ** 3 / 3, abs(timediff) ** 2 / 2],
                [abs(timediff) ** 2 / 2, abs(timediff)],
            ]
        )
        * noise_diff_coeff
    )
    ...
    assert np.array_equal(F, cv.matrix(timestamp=new_timestamp, time_interval=time_interval))

    # Ensure ```cv.covar(time_interval)``` returns Q
    assert np.array_equal(Q, cv.covar(timestamp=new_timestamp, time_interval=time_interval))
