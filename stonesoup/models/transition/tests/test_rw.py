...
def test_rwodel(sign):
    ...
    assert np.array_equal(F, rw.matrix(timestamp=new_timestamp, time_interval=time_interval))

    # Ensure ```rw.covar(time_interval)``` returns Q
    assert np.array_equal(Q, rw.covar(timestamp=new_timestamp, time_interval=time_interval))
