...
def test_oumodel(sign):
    ...
    F = np.array([[1, (1 - exp_kdt) / k], [0, exp_kdt]])

    q11 = q / k**2 * (dt - 2 / k * (1 - exp_kdt) + 1 / (2 * k) * (1 - exp_2kdt)) * sign
    q12 = q / k * ((1 - exp_kdt) / k - 1 / (2 * k) * (1 - exp_2kdt))
    ...
    with pytest.raises(
        TypeError,
        match="'noise_diff_coeff' should be a "
    ...
    assert np.allclose(
        F, ou.matrix(timestamp=new_timestamp, time_interval=time_interval), rtol=1e-10
    )
    # Ensure ```ou.covar(time_interval)``` returns Q
    assert np.allclose(
        Q, ou.covar(timestamp=new_timestamp, time_interval=time_interval), rtol=1e-10
    )
    ...
    assert np.allclose(new_state_vec_wo_noise, F @ state.state_vector, rtol=1e-10)
    ...
    assert approx(prob) == multivariate_normal.pdf(
        new_state_vec_wo_noise.T, mean=np.array(F @ state.state_vector).ravel(), cov=Q
    )
    ...
    assert not np.allclose(new_state_vec_w_inoise, F @ state.state_vector, rtol=1e-10)
    ...
    assert approx(prob) == multivariate_normal.pdf(
        new_state_vec_w_inoise.T, mean=np.array(F @ state.state_vector).ravel(), cov=Q
    )
    ...
    assert np.allclose(new_state_vec_w_enoise, F @ state.state_vector + noise, rtol=1e-10)
    ...
    assert approx(prob) == multivariate_normal.pdf(
        new_state_vec_w_enoise.T, mean=np.array(F @ state.state_vector).ravel(), cov=Q
    )
