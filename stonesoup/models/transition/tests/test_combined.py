...
def test__linear_combined(comb_model):
    F = 3 * np.eye(3)
    Q = 3 * np.eye(3)
    model_1 = LinearGaussianTimeInvariantTransitionModel(transition_matrix=F, covariance_matrix=Q)
    ...
    combined_model = comb_model([model_1, model_2, model_3, model_4])
    ...
    assert combined_model.ndim_state == DIM
    assert combined_model.jacobian(State(x_prior), time_interval=t_delta).shape == (DIM, DIM)
    assert combined_model.covar(time_interval=t_delta).shape == (DIM, DIM)
    assert combined_model.function(
        State(x_prior), noise=np.random.randn(DIM, 1), time_interval=t_delta
    ).shape == (DIM, 1)
    # Test vectorized handling i.e. multiple state vector inputs
    assert state.state_vector.shape == combined_model.function(state, time_interval=t_delta).shape
    assert combined_model.rvs(time_interval=t_delta).shape == (DIM, 1)
    assert isinstance(
...
def test_nonlinear_combined(model_4):
    ...
    assert combined_model.ndim_state == DIM
    assert combined_model.covar(time_interval=t_delta).shape == (DIM, DIM)
    assert combined_model.function(State(x_prior), noise=True, time_interval=t_delta).shape == (
        DIM,
        1,
    )
    # Test vectorized handling i.e. multiple state vector inputs
