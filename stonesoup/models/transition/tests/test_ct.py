...
def test_ctmodel(sign):
    ...
    F = np.array(
        [
            [1, np.sin(turn_ratedt) / turn_rate, 0, -(1 - np.cos(turn_ratedt)) / turn_rate],
            [0, np.cos(turn_ratedt), 0, -np.sin(turn_ratedt)],
            [0, (1 - np.cos(turn_ratedt)) / turn_rate, 1, np.sin(turn_ratedt) / turn_rate],
            [0, np.sin(turn_ratedt), 0, np.cos(turn_ratedt)],
        ]
    )
    ...
    Q = np.array(
        [
            [qx * abs(timediff) ** 3 / 3, qx * abs(timediff) ** 2 / 2, 0, 0],
            [qx * abs(timediff) ** 2 / 2, qx * abs(timediff), 0, 0],
            [0, 0, qy * abs(timediff) ** 3 / 3, qy * abs(timediff) ** 2 / 2],
            [0, 0, qy * abs(timediff) ** 2 / 2, qy * abs(timediff)],
        ]
    )

    # Ensure ```model_obj.transfer_function(time_interval)``` returns F
    assert np.array_equal(F, model_obj.jacobian(State(state_vec), time_interval=time_interval))
