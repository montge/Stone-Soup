...
def singer_model_params(request):
    if request.param == 1:
        ...
        state = State(np.array([[3.0], [1.0], [0.1], [2.0], [2.0], [0.2], [4.0], [0.5], [0.05]]))
...
def test_singer(singer_model_params, sign):
    ...
    if dim == 1:
        ...
        with pytest.raises(
            ...
            Singer(noise_diff_coeff=[noise_diff_coeffs[0]], damping_coeff=damping_coeffs[0])
        model_obj = Singer(noise_diff_coeff=noise_diff_coeffs[0], damping_coeff=damping_coeffs[0])
    else:
        model_list = [
            Singer(noise_diff_coeff=noise_diff_coeffs[i], damping_coeff=damping_coeffs[i])
        ...
        mat_list.append(
            np.array(
                [
                    ...
                    [0, 1, (1 - np.exp(-damping_coeffdt)) / damping_coeff],
                    [0, 0, np.exp(-damping_coeffdt)],
                ]
            )
        )
            ...
            np.array(
                [
                    [
                        ...
                        * sign,
                        (alpha_time - (1 - e_neg_at)) ** 2 / (2 * damping_coeff**4),
                    ...
                    [
                        ...
                        / (2 * damping_coeff**3)
                        * sign,
                        (1 - e_neg_at) ** 2 / (2 * damping_coeff**2),
                    ],
                    [
                        ((1 - e_neg2_at) - 2 * alpha_time * e_neg_at)
                        / (2 * damping_coeff**3)
    ...
    assert np.allclose(
        F, model_obj.jacobian(State(state_vec), time_interval=time_interval), rtol=1e-6
    )

    # Ensure ```model_obj.covar(time_interval)``` returns Q
    assert np.allclose(
        Q, model_obj.covar(timestamp=new_timestamp, time_interval=time_interval), rtol=1e-6
    )
