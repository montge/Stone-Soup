...
def test_isotropic_plume(state, mapping, translation_offset):
    ...
    if conc.state_vector[0] <= sensing_threshold:
        actual_likelihood = (1 - missed_detection_probability) * 1 / 2 * (
            1 + erf((sensing_threshold - pred_conc) / (sensing_threshold * np.sqrt(2)))
