...
@pytest.mark.parametrize(
    "control_input, control_model",
    [
        (State(StateVector([1])), LinearControlModel(np.array([[0.25], [1]]))),
        (
            State(StateVector([1, 2, 5])),
            LinearControlModel(
                np.array(
                    [[0.25, 0, 0], [1, 0, 0], [0, 0.25, 0], [0, 1, 0], [0, 0, 0.25], [0, 0, 1]]
                ),
                control_noise=np.eye(3),
            ),
        ),
    ],
...
def test_linear_model(control_input, control_model):
    ...
    assert np.all(
        np.isclose(
            control_model.function(control_input, noise=False),
