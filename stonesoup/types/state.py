...
class State(Type):
    ...
    @staticmethod
    def from_state(
        state: "State", *args: Any, target_type: Type | None = None, **kwargs: Any
    ) -> "State":
...
class StateMutableSequence(Type, MutableSequence):
    ...
    def __getitem__(self, index):
        if isinstance(index, slice) and (
            isinstance(index.start, datetime.datetime) or isinstance(index.stop, datetime.datetime)
        ):
            items = []
...
GaussianState.register(SqrtGaussianState)
...
class InformationState(State):
    ...
    def from_gaussian_state(cls, gaussian_state, *args, **kwargs):
        ...
        return cls(
            state_vector=state_vector, precision=precision, timestamp=timestamp, *args, **kwargs
        )
...
class WeightedGaussianState(GaussianState):
    ...
    def from_gaussian_state(cls, gaussian_state, *args, copy=True, **kwargs):
        ...
        if copy:
            ...
            covar = covar.copy()
        return cls(state_vector=state_vector, covar=covar, timestamp=timestamp, *args, **kwargs)
...
TaggedWeightedGaussianState.register(ASDTaggedWeightedGaussianState)
...
class ParticleState(State):
    ...
    def __init__(self, *args, **kwargs):
        weight = next(
            (
                val
                for name, val in zip(type(self).properties, args, strict=False)
                if name == "weight"
            ),
            kwargs.get("weight"),
        )
        log_weight, idx = next(
            (
                (val, idx)
                for idx, (name, val) in enumerate(zip(type(self).properties, args, strict=False))
                if name == "log_weight"
            ),
            (kwargs.get("log_weight"), None),
        )
    ...
    def __getitem__(self, item):
        ...
        log_weight = self.log_weight[item] if self.log_weight is not None else None
        ...
        if isinstance(item, int):
            ...
            result = type(self).from_state(
                self, state_vector=self.state_vector[:, item], log_weight=log_weight, parent=parent
            )
        return result
    ...
    def from_state(
        ...
        particle_list, particle_list_idx = next(
            (
                (val, idx)
                for idx, (name, val) in enumerate(zip(cls.properties, args, strict=False))
                if name == "particle_list"
            ),
            (kwargs.get("particle_list"), None),
        )
    ...
    def mean(self):
        ...
        if len(self) == 1:  # No need to calculate mean
            return self.state_vector
        return np.average(
            self.state_vector, axis=1, weights=np.exp(self.log_weight - np.max(self.log_weight))
        )
    ...
    def covar(self):
        ...
        return np.cov(
            self.state_vector, ddof=0, aweights=np.exp(self.log_weight - np.max(self.log_weight))
        )
...
class MultiModelParticleState(ParticleState):
    ...
    def __getitem__(self, item):
        ...
        dynamic_model = self.dynamic_model[item] if self.dynamic_model is not None else None
...
class EnsembleState(State):
    ...
    def generate_ensemble(mean, covar, num_vectors):
        ...
        ndim = mean.shape[0]
        vectors = np.atleast_2d(multivariate_normal.rvs(np.zeros(ndim), covar, num_vectors))
        if ndim > 1:
            vectors = vectors.T
...
class CategoricalState(State):
    ...
    def __str__(self):
        strings = [
            f"P({category}) = {p}"
            for category, p in zip(self.categories, self.state_vector, strict=False)
        ]
        string = ",\n".join(strings)
        return string
