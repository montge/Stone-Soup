...
def test_GaussianMixture():
    ...
    states = [
        WeightedGaussianState(
            state_vector=np.random.rand(dim, 1), covar=np.eye(dim), weight=np.random.rand()
        )
        for _ in range(num_states)
    ]
    mixturestate = GaussianMixture(components=states)
    assert len(mixturestate) == num_states
    for component1, component2 in zip(mixturestate, states, strict=False):
...
def test_GaussianMixture_with_single_component():
    ...
    state = TaggedWeightedGaussianState(
        state_vector=np.random.rand(dim, 1), covar=np.eye(dim), weight=np.random.rand(), tag=2
    )
...
def test_GaussianMixture_timestamp():
    ...
    state = WeightedGaussianState(
        state_vector=np.random.rand(dim, 1), covar=np.eye(dim), timestamp=timestamp
    )
...
def test_GaussianMixture_get_and_set_item():
    ...
    states = [
        WeightedGaussianState(
            state_vector=np.random.rand(dim, 1), covar=np.eye(dim), weight=weights[i]
        )
        for i in range(num_states)
    ]
    mixturestate = GaussianMixture(components=states)
    new_component = WeightedGaussianState(
        state_vector=np.random.rand(dim, 1), covar=np.eye(dim), weight=1
    )
...
def test_GaussianMixture_contains_item():
    ...
    check_component_not_in = WeightedGaussianState(
        state_vector=np.random.rand(dim, 1), covar=np.eye(dim), weight=np.random.rand(1, 1)
    )
...
def test_GaussianMixture_Gaussian_state():
    ...
    states = [
        WeightedGaussianState(
            state_vector=[float(i)] * dim, covar=np.eye(dim), weight=1.0, timestamp=timestamp
        )
