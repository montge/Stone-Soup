...
def test_track_state(state):
    ...
    if hasattr(state, "particles"):
        for track_particle, state_particle in zip(track.particles, state.particles, strict=False):
        ...
        hypothesis=MultipleHypothesis(
            [
                SingleProbabilityHypothesis(
                    None, Detection(np.array([[0]]), metadata={"colour": "red"}), probability=0.8
                ),
                SingleProbabilityHypothesis(
                    None,
        ...
        hypothesis=MultipleHypothesis(
            [
                SingleDistanceHypothesis(
                    None, Detection(np.array([[0]]), metadata={"colour": "maroon"}), distance=1
                ),
                SingleDistanceHypothesis(
                    None, Detection(np.array([[0]]), metadata={"colour": "lime"}), distance=2
                ),
            ]
        )
    )
...
def test_copy():
    ...
    track = Track([State([0, 1]), State([1, 2]), State([3, 4])], init_metadata=metadatas[0])
    ...
    for original_state, copied_state in zip(track.states, copied_track.states, strict=False):
        assert original_state is copied_state

    for original_metadata, copied_metadata in zip(
        track.metadatas, copied_track.metadatas, strict=False
    ):
