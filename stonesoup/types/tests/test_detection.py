...
def test_composite_detection():
    ...
    with pytest.raises(ValueError, match="Mappings and sub-detections must have same count"):
        CompositeDetection(
            sub_states=[Detection([0]), Detection([0]), Detection([0])], mapping=[0, 1]
        )

    with pytest.raises(ValueError, match="Mappings and sub-detections must have same count"):
        CompositeDetection(
            sub_states=[Detection([0]), Detection([0]), Detection([0])], mapping=[0, 1, 2, 3]
        )
    ...
    detections = [a, b, c, d]
    detection = CompositeDetection(sub_states=detections, mapping=[1, 0, 2, 3])
