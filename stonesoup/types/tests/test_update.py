...
def test_stateupdate():
    ...
    prediction = StatePrediction(
        state_vector=np.array([[1], [1], [0.5], [0.5]]), timestamp=timestamp
    )
    measurement_prediction = StateMeasurementPrediction(
        state_vector=np.array([[2], [2], [1.5], [1.5]]), timestamp=timestamp
    )
    measurement = Detection(state_vector=np.array([[5], [7]]), timestamp=timestamp)
    ...
    assert np.array_equal(measurement_prediction, state_update.hypothesis.measurement_prediction)
...
def test_gaussianstateupdate():
    ...
    measurement = Detection(state_vector=np.array([[5], [7]]), timestamp=timestamp)
    state_update = GaussianStateUpdate(
        state_vector,
        covar,
        SingleHypothesis(
            prediction=prediction, measurement=measurement, measurement_prediction=meas_pred
        ),
        timestamp=timestamp,
    )
    ...
    assert np.array_equal(prediction, state_update.hypothesis.prediction)
    assert np.array_equal(meas_pred, state_update.hypothesis.measurement_prediction)
    assert np.array_equal(measurement, state_update.hypothesis.measurement)
...
def test_asdgaussianstateupdate():
    ...
    prediction = ASDGaussianStatePrediction(
        multi_state_vector=state_vector,
        multi_covar=covar,
    ...
    measurement = Detection(state_vector=np.array([[5], [7]]), timestamp=timestamp)
    state_update = ASDGaussianStateUpdate(
        multi_state_vector=state_vector,
        multi_covar=covar,
...
def test_particlestateupdate():
    ...
    eval_mean = np.mean(np.hstack([i.state_vector for i in particles]), axis=1).reshape(2, 1)
    assert np.allclose(eval_mean, state_update.mean)
    assert np.all(
        [
            particles[i].state_vector == StateVector(state_update.state_vector[:, i])
            for i in range(9)
        ]
    )
...
def test_from_state():
    ...
    update = Update.from_state(
        state, None, particle_list=[Particle([[1]], weight=0.5)], hypothesis=None
    )
...
def test_kernel_particle_state_update():
    ...
    prediction = KernelParticleStatePrediction(state_vector, weight=weights, timestamp=timestamp)
