...
def params():
    ...
    state_vector = [
        np.random.uniform(-range_value, range_value, 1),
    ...
    for i in range(number_of_targets):
        ...
        truthlet_lengths = np.random.choice(
            range(min_segment_length, max_segment_length), number_of_segments
        )
        disjoint_lengths = np.random.choice(
            range(min_disjoint_length, max_disjoint_length), number_of_segments
        )
        segment_pair_lengths = np.insert(
            truthlet_lengths + disjoint_lengths, 0, truthlet0_length, axis=0
        )
        ...
        truth = GroundTruthPath([GroundTruthState(state_vector, timestamp=start_time)], id=i)
        ...
        for j in range(number_of_segments):
            truthlet = GroundTruthPath(
                truth[starts[j] : stops[j]],
                id=str("G::" + str(truth.id) + "::S::" + str(j) + "::"),
            )
            truthlets.add(truthlet)
        truths.add(truth)

    for _n, truthlet in enumerate(truthlets):
        measurementlet = []
        ...
        tracklet = MultiTargetTracker(
            initiator=initiator,
            deleter=deleter,
            detector=measurementlet,
            data_associator=data_associator,
            updater=updater,
        )
        for _, t in tracklet:
...
def test_correct_no_stitched_tracks2(params):
    ...
    all_tracks = params["all_tracks"]
    start_time = params["start"]
    stitcher2 = TrackStitcher(
        forward_hypothesiser=hypothesiser, backward_hypothesiser=hypothesiser
    )
