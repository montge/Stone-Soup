...
def test_voxel_updater_update_with_detection_dense(
    ...
    hypothesis = SingleHypothesis(
        prediction=dense_voxel_prediction,
        measurement=detection_near_center
    )
...
def test_voxel_updater_update_with_detection_sparse(
    ...
    hypothesis = SingleHypothesis(
        prediction=sparse_voxel_prediction,
        measurement=detection_near_center
    )
...
def test_voxel_updater_increases_occupancy_near_detection(
        measurement_model, small_voxel_grid):
...
def test_voxel_updater_detection_probability_effect(
        measurement_model, small_voxel_grid):
    ...
    updater_low_pd = VoxelUpdater(
        measurement_model=measurement_model,
        detection_probability=0.5
    )

    # High detection probability
    updater_high_pd = VoxelUpdater(
        measurement_model=measurement_model,
...
def test_voxel_updater_multiple_detections_sequence(
        measurement_model, small_voxel_grid):
...
def test_voxel_updater_likelihood_computation(measurement_model, small_voxel_grid):
    ...
    hypothesis_center = SingleHypothesis(
        prediction=prediction, measurement=detection_at_center)
    hypothesis_far = SingleHypothesis(
        prediction=prediction, measurement=detection_far)
...
def test_voxel_updater_bayesian_update_consistency(
        measurement_model, small_voxel_grid):
...
def test_voxel_updater_sparse_to_dense_consistency(
        measurement_model, voxel_grid):
    ...
    hypothesis_sparse = SingleHypothesis(
        prediction=sparse_prediction, measurement=detection)
    hypothesis_dense = SingleHypothesis(
        prediction=dense_prediction, measurement=detection)
    ...
    for idx in sparse_occupancy.keys():
...
def test_voxel_updater_high_detection_probability_edge_case(
        measurement_model, small_voxel_grid):
...
def test_voxel_updater_repeated_updates_convergence(
        measurement_model, small_voxel_grid):
...
def test_voxel_updater_different_measurement_models(small_voxel_grid):
    ...
    low_noise_model = LinearGaussian(
        ndim_state=3,
        mapping=[0, 1, 2],
        noise_covar=CovarianceMatrix(np.eye(3) * 0.01)
    )
    ...
    high_noise_model = LinearGaussian(
        ndim_state=3,
        mapping=[0, 1, 2],
        noise_covar=CovarianceMatrix(np.eye(3) * 1.0)
    )

    updater_low_noise = VoxelUpdater(
        measurement_model=low_noise_model,
        detection_probability=0.9
    )

    updater_high_noise = VoxelUpdater(
        measurement_model=high_noise_model,
        detection_probability=0.9
    )
