...
from stonesoup.updater.kalman import ExtendedKalmanUpdater, KalmanUpdater, UnscentedKalmanUpdater
from stonesoup.updater.pointprocess import LCCUpdater, PHDUpdater
...
def test_phd_single_component_update(UpdaterClass, measurement_model, prediction, measurement):
    ...
    hypotheses = [
        MultipleHypothesis([SingleHypothesis(prediction=prediction, measurement=measurement)]),
        MultipleHypothesis([SingleHypothesis(prediction=prediction, measurement=None)]),
    ]
    ...
    assert np.allclose(updated_component.mean, eval_posterior.mean, 0, atol=1.0e-14)
    assert np.allclose(updated_component.covar, eval_posterior.covar, 0, atol=1.0e-14)
    ...
    assert np.allclose(miss_detected_component.mean, prediction.mean, 0, atol=1.0e-14)
    assert np.allclose(miss_detected_component.covar, prediction.covar, 0, atol=1.0e-14)
...
def test_lcc_single_component_update(UpdaterClass, measurement_model, prediction, measurement):
    eval_measurement_prediction = GaussianMeasurementPrediction(
        measurement_model.matrix() @ prediction.mean,
        measurement_model.matrix() @ prediction.covar @ measurement_model.matrix().T
        + measurement_model.covar(),
