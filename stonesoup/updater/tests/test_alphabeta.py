...
from stonesoup.types.prediction import StateMeasurementPrediction, StatePrediction
...
def test_alphabeta(measurement_model, prediction, measurement, alpha, beta):
    ...
    eval_posterior_position = prediction.state_vector[[0, 2]] + alpha * (
    ...
    updater = AlphaBetaUpdater(measurement_model=measurement_model, alpha=alpha, beta=beta)
    ...
    posterior = updater.update(
        SingleHypothesis(prediction=prediction, measurement=measurement), time_interval=timediff
    )

    assert np.allclose(posterior.state_vector, eval_posterior.state_vector, 0, atol=1.0e-14)
