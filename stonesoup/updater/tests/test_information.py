...
def test_information(UpdaterClass, measurement_model, prediction, measurement):
    ...
    assert np.allclose(kposterior.covar, np.linalg.inv(posterior.precision), 0, atol=1.0e-14)
    ...
    meas_model_winv = LinearGaussianwithInverse(
    ...
    # and check
    assert np.allclose(posterior.state_vector, post_from_inv.state_vector, 0, atol=1.0e-14)
    ...
    assert np.allclose(
        posterior.precision - posterior.precision.T,
