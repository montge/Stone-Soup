...
def test_ensemble(ensemble_updater, model_on_detection):
    ...
    measurement_model.noise_covar += 1.0  # Increase noise to confirm test
    updater = ensemble_updater(None) if model_on_detection else ensemble_updater(measurement_model)
    ...
    assert np.sum(np.trace(no_noise_measurement_prediction.covar)) < np.sum(
        np.trace(noise_measurement_prediction.covar)
    )
