...
from ...resampler.particle import ESSResampler, SystematicResampler
...
from ...types.state import MultiModelParticleState, RaoBlackwellisedParticleState
from ...types.update import MultiModelParticleStateUpdate, RaoBlackwellisedParticleStateUpdate
from ...updater.particle import MultiModelParticleUpdater, RaoBlackwellisedParticleUpdater


@pytest.fixture()
def dynamic_model_list():
    return [
        CombinedLinearGaussianTransitionModel((ConstantVelocity(0.01), ConstantVelocity(0.01))),
        CombinedLinearGaussianTransitionModel(
...
def position_mappings():
    return [[0, 1, 3, 4], [0, 1, 2, 3, 4, 5], [1, 2, 3, 4]]


@pytest.fixture()
def transition_matrix():
    return [[0.40, 0.40, 0.2], [0.40, 0.40, 0.2], [0.40, 0.40, 0.2]]
...
def test_multi_model(
    ...
    state_vector = StateVectors(
        multivariate_normal.rvs([1, 1, 0.5, 1, 1, 0.5], np.diag([0.2, 0.07, 0.02] * 2), 3000).T
    )
    ...
    detection = Detection([[0.5, 7.0]], timestamp, measurement_model=measurement_model)
...
def test_rao_blackwellised(
    ...
    particle3 = RaoBlackwellisedParticle(
        state_vector=[1, 1, 0.5, 1, 1, 0.5], weight=1 / 3000, model_probabilities=[0.2, 0.2, 0.6]
    )
    ...
    # Detection close to where known turn rate model would place particles
    detection = Detection([[0.5, 7.0]], timestamp)
