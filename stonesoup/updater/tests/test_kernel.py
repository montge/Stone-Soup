...
samples = multivariate_normal.rvs(
    [0, 0, 0, 0], np.diag([0.01, 0.005, 0.1, 0.5]) ** 2, size=number_particles
)
weights = np.array([1 / number_particles] * number_particles)
prior = KernelParticleState(
    state_vector=StateVectors(samples.T), weight=weights, timestamp=timestamp
)
samples = multivariate_normal.rvs(
    [0, 0, 0, 0], np.diag([0.01, 0.005, 0.1, 0.5]) ** 2, size=number_particles
)
proposal = KernelParticleState(
    state_vector=StateVectors(samples.T), weight=weights, timestamp=timestamp
)
...
prediction = predictor.predict(
    prior=prior, proposal=proposal, timestamp=new_timestamp, noise=False
)
...
@pytest.mark.parametrize(
    ...
    [
        (  # No Kernel
            None,
            Cartesian2DToBearing(ndim_state=4, mapping=(0, 2), noise_covar=np.array([[0.005**2]])),
            1,
            10,
        ),
        (QuadraticKernel(), None, 1, 10),  # With Kernel
    ],
    ids=[
        "measurement_model",
...
def test_kernel_updater(kernel, measurement_model, c, ialpha):
    ...
    mm = Cartesian2DToBearing(ndim_state=4, mapping=(0, 2), noise_covar=np.array([[0.005**2]]))
    measurement = Detection(
        mm.function(gt_state, noise=True), timestamp=gt_state.timestamp, measurement_model=mm
    )
    updater = AdaptiveKernelKalmanUpdater(
        measurement_model=measurement_model, kernel=QuadraticKernel()
    )
    ...
    Q_AKKF = prediction.kernel_covar @ np.linalg.pinv(
        G_yy @ prediction.kernel_covar + updater.lambda_updater * np.identity(len(prediction))
    )

    updated_weights = np.atleast_2d(prediction.weight).T + Q_AKKF @ (
        g_y - np.atleast_2d(G_yy @ prediction.weight).T
    )
    updated_covariance = prediction.kernel_covar - Q_AKKF @ G_yy @ prediction.kernel_covar

    # Proposal Calculation
    pred_mean = prediction.state_vector @ np.squeeze(updated_weights)
    pred_covar = np.diag(
        np.diag(prediction.state_vector @ updated_covariance @ prediction.state_vector.T)
    )

    new_state_vector = multivariate_normal.rvs(pred_mean, pred_covar, size=len(prediction))
    ...
    assert np.allclose(
        np.mean(update.proposal, axis=1), np.mean(StateVectors(new_state_vector.T), axis=1), atol=2
    )
...
@pytest.mark.parametrize(
    ...
    ids=["no_kernel", "kernel", "c2i20"],
)
def test_no_kernel(updater_class, kernel, measurement_model, c, ialpha):
    updater = updater_class(measurement_model=measurement_model, kernel=kernel)
...
def test_no_measurement_model():
    ...
    updater = AdaptiveKernelKalmanUpdater(measurement_model=mm, kernel=QuadraticKernel())
    gt_state = GroundTruthState([-0.1, 0.001, 0.7, -0.055], timestamp=new_timestamp)
