...
@pytest.mark.parametrize(
    ...
    [
        (  # Chernoff Updater
            ChernoffUpdater,
            LinearGaussian(ndim_state=2, mapping=[0, 1], noise_covar=np.array([[0.04, 0.04]])),
            GaussianStatePrediction(
                np.array([[-6.45], [0.7]]), np.array([[4.1123, 0.0013], [0.0013, 0.0365]])
            ),
            GaussianDetection(state_vector=np.array([[-6.23, 0.83]]), covar=np.diag([0.75, 1.2])),
            0.5,
        )
    ],
    ids=["standard"],
...
def test_chernoff(UpdaterClass, measurement_model, prediction, measurement, omega):
    ...
    posterior_cov = np.linalg.inv(
        omega * np.linalg.inv(measurement.covar) + (1 - omega) * np.linalg.inv(prediction.covar)
    )
    posterior_mean = posterior_cov @ (
        omega * np.linalg.inv(measurement.covar) @ measurement.state_vector
        + (1 - omega) * np.linalg.inv(prediction.covar) @ prediction.state_vector
    )
    eval_posterior = GaussianState(posterior_mean, posterior_cov)
    ...
    assert np.allclose(measurement_prediction.covar, prediction.covar, 0, atol=1.0e-14)
    assert np.allclose(
