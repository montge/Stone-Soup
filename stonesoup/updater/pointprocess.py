...
class PointProcessUpdater(Base):
    ...
    normalisation: bool = Property(default=True, doc="Flag for normalisation")
    prob_detection: Probability = Property(
        default=1, doc="Probability of a target being detected at the current timestep"
    )
    prob_survival: Probability = Property(
        default=1, doc="Probability of a target surviving until the next timestep"
    )
    ...
    def update(self, hypotheses):
        ...
        for multi_hypothesis in hypotheses[:-1]:
            ...
            for hypothesis in multi_hypothesis:
                ...
                new_weight = self.prob_detection * prediction.weight * q
        ...
        return GaussianMixtureUpdate(hypothesis=hypotheses, components=updated_components)
...
class LCCUpdater(PointProcessUpdater):
    ...
    mean_number_of_false_alarms: float = Property(
        default=1, doc="Mean number of false alarms (clutter) expected per timestep"
    )
    variance_of_false_alarms: float = Property(
        default=1, doc="Variance on the number of false alarms (clutter) expected per timestep"
    )
    ...
    def _calculate_update_terms(self, updated_sum_list, hypotheses):
        ...
        predicted_weight_sum = (
            Probability.sum(hypothesis.prediction.weight for hypothesis in hypotheses[-1])
            * self.prob_survival
        )
        ...
        denominator = alpha_pred + detected_weight_sum + self.mean_number_of_false_alarms
        ...
        detected_c2 = Probability.sum(
            [
                weight_sum / ((weight_sum + self.clutter_spatial_density) ** 2)
