...
def test_csv_gt_2d(csv_gt_filename):
    ...
    csv_reader = CSVGroundTruthReader(
        csv_gt_filename.strpath,
...
def test_csv_gt_3d_timestamp_csv_opt(tmpdir):
    ...
    csv_reader = CSVGroundTruthReader(
        ...
        timestamp=True,
        path_id_field="identifier",
        csv_options={"fieldnames": ["x", "y", "z", "identifier", "t"]},
    )
...
def test_csv_default(csv_det_filename):
    ...
    detections = [detection for _, detections in csv_reader for detection in detections]
...
def test_csv_metadata_time(csv_det_filename):
    ...
    csv_reader = CSVDetectionReader(
        csv_det_filename.strpath,
        ["x", "y"],
        "t",
        time_field_format="%Y-%m-%dT%H:%M:%SZ",
        metadata_fields=["z"],
...
def test_csv_missing_metadata_timestamp(tmpdir):
    ...
    csv_reader = CSVDetectionReader(
        csv_det_filename.strpath, ["x", "y"], "t", metadata_fields=["heading"], timestamp=True
    )
    detections = [detection for _, detections in csv_reader for detection in detections]
...
def test_tsv(tmpdir):
    ...
    #   - copy all metadata items
    csv_reader = CSVDetectionReader(
        csv_filename.strpath, ["x", "y"], "t", csv_options={"dialect": "excel-tab"}
    )
    detections = [detection for _, detections in csv_reader for detection in detections]
...
def test_dict_gt_reader():
    gt_dict_test = [
        {"name": "Red Ten", "x": 0, "y": 0, "t": datetime.datetime(2025, 5, 12, 0, 0, 0)},
        {"name": "Red Ten", "x": 0, "y": 1, "t": datetime.datetime(2025, 5, 12, 0, 0, 1)},
        {"name": "Red Five", "x": 1, "y": 0, "t": datetime.datetime(2025, 5, 12, 0, 0, 1)},
        {"name": "Red Ten", "x": 0, "y": 2, "t": datetime.datetime(2025, 5, 12, 0, 0, 2)},
        {"name": "Red Five", "x": 2, "y": 0, "t": datetime.datetime(2025, 5, 12, 0, 0, 2)},
    ]
...
def test_get_time(dictionary_track_reader: DictionaryTrackReader):
    ...
    assert dictionary_track_reader._get_time(row) == expected_time
...
def test_get_metadata(dictionary_track_reader: DictionaryTrackReader):
    ...
    assert dictionary_track_reader._get_metadata(row) == expected_metadata
