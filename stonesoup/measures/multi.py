...
class StateSequenceMeasure(MultipleMeasure):
    ...
    def __call__(
        self,
        state_sequence_1: StateMutableSequence,
        state_sequence_2: StateMutableSequence,
        times_to_measure: Sequence[datetime] | None = None,
    ) -> list[float]:
        """
        ...
        if times_to_measure is None:
            ...
            if len(times_to_measure) == 0:
                warnings.warn(
                    "No measures are calculated as there are not any times that match "
                    "between the two state sequences.",
                    stacklevel=2,
                )

        measures = [
            self.state_measure(state_sequence_1[time], state_sequence_2[time])
...
class RecentStateSequenceMeasure(MultipleMeasure):
    ...
    def __call__(
        self, state_sequence_1: StateMutableSequence, state_sequence_2: StateMutableSequence
    ) -> list[float]:
        """
