...
def test_multi_model():
    ...
    # Define prior state.
    prior_vectors = np.full((9, 10), 10.0).view(StateVectors)
    ...
    model_list = [
        ...
        CombinedLinearGaussianTransitionModel(
            (ConstantAcceleration(0.01), ConstantAcceleration(0.01), ConstantAcceleration(0.01))
        ),
        CombinedLinearGaussianTransitionModel(
        ...
        CombinedLinearGaussianTransitionModel(
            (ConstantVelocity(0.01), ConstantVelocity(0.01), ConstantAcceleration(0.01))
        ),
        CombinedLinearGaussianTransitionModel(
            (ConstantAcceleration(0.01), ConstantVelocity(0.01), ConstantVelocity(0.01))
        ),
        CombinedLinearGaussianTransitionModel(
            (ConstantVelocity(0.01), ConstantAcceleration(0.01), ConstantVelocity(0.01))
        ),
        CombinedLinearGaussianTransitionModel(
            (ConstantAcceleration(0.01), ConstantAcceleration(0.01), ConstantVelocity(0.01))
        ),
    ]
    ...
    predictor = MultiModelPredictor(
        model_mappings=model_mappings, transition_matrix=transition, transition_models=model_list
    )
