...
prior_update = KernelParticleStateUpdate(
...
def test_kernel(predictor_class, kernel, transition_model, prior_state, proposal_state):
    ...
    prediction = predictor.predict(
        prior=prior_state, proposal=proposal_state, timestamp=new_timestamp, noise=False
    )
    if kernel is None:
    ...
    new_state_vector = transition_model.function(
        proposal_state, noise=False, time_interval=time_diff
    )
    k_tilde_tilde = kernel(proposal_state)
    k_tilde_nontilde = kernel(proposal_state, prior)

    kernel_t = (
        np.linalg.pinv(k_tilde_tilde + predictor.lambda_predictor * np.identity(len(prior)))
        @ k_tilde_nontilde
    )
    ...
    v = (
        (
            np.linalg.pinv(k_tilde_tilde + predictor.lambda_predictor * np.identity(len(prior)))
            @ k_tilde_tilde
            - np.identity(len(prior))
        )
        @ (
            np.linalg.inv(k_tilde_tilde + predictor.lambda_predictor * np.identity(len(prior)))
            @ k_tilde_tilde
            - np.identity(len(prior))
        ).T
        / len(prior)
