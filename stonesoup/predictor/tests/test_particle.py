...
from ...types.prediction import BernoulliParticleStatePrediction, ParticleStatePrediction
from ...types.state import BernoulliParticleState, ParticleState, State
from ...types.update import BernoulliParticleStateUpdate


@pytest.mark.parametrize("predictor_class", (ParticlePredictor, ParticleFlowKalmanPredictor))
def test_particle(predictor_class):
    ...
    assert np.all(
        [eval_prediction.state_vector[:, i] == prediction.state_vector[:, i] for i in range(9)]
    )
...
def test_bernoulli_particle_no_detection(predictor, extra_params):
    ...
    eval_prior.state_vector = np.concatenate((eval_prior.state_vector, birth_samples.T), axis=1)
    ...
    if len(extra_params) > 0:
        ...
        eval_weight[:9] = (
            np.log((survival_prob * existence_prob) / eval_existence)
            + (log_likelihood[:9] - original_log_likelihood[:9])
            + np.log(eval_weight[:9])
        )
        eval_weight[9:] = (
            np.log((birth_prob * (1 - existence_prob)) / eval_existence)
            + (log_likelihood[9:] - original_log_likelihood[9:])
            + np.log(eval_weight[9:])
        )
...
def test_bernoulli_particle_detection(predictor, extra_params):
    ...
    eval_prior = copy.copy(prior)
    eval_prior.state_vector = np.concatenate((eval_prior.state_vector, birth_samples), axis=1)
    eval_prior.weight = np.array([1 / 18] * 18)
...
def test_smcphd(birth_scheme):
    ...
    birth_sampler = ParticleSampler(
        distribution_func=rng.multivariate_normal,
