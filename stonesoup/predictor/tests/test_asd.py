...
def test_asdkalman():
    ...
    # Define prior state
    prior = ASDGaussianState(prior_mean, multi_covar=prior_covar, timestamps=timestamp)

    # Calculate evaluation variables
    eval_prediction = ASDGaussianStatePrediction(
        transition_model.matrix(timestamp=new_timestamp, time_interval=time_interval) @ prior.mean,
        multi_covar=transition_model.matrix(timestamp=new_timestamp, time_interval=time_interval)
        @ prior.covar
        @ transition_model.matrix(timestamp=new_timestamp, time_interval=time_interval).T
        + transition_model.covar(timestamp=new_timestamp, time_interval=time_interval),
        timestamps=[new_timestamp],
        act_timestamp=new_timestamp,
    )
