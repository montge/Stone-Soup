...
class ASDKalmanPredictor(KalmanPredictor):
    ...
    def _predict_over_interval(self, prior, timestamp):
        ...
        if predict_over_interval.total_seconds() < 0:
            predict_over_interval, timestamp_from_which_is_predicted = min(
                (
                    (timestamp - t, t)
                    for t in prior.timestamps
                    if (timestamp - t).total_seconds() >= 0
                ),
                key=itemgetter(0),
            )
    ...
    def predict(self, prior, timestamp, **kwargs):
        ...
        **kwargs :
        ...
        predict_over_interval, timestamp_from_which_is_predicted = self._predict_over_interval(
            prior, timestamp
        )
        ...
        if t_index == 0:
            ...
            correlation_matrices[t_index] = time_corr_matrices = correlation_matrices[
                t_index
            ].copy()
            ...
            transition_matrix_m_plus_1 = self._transition_matrix(
                prior=state_pred_m, time_interval=time_interval_m_to_m_plus_1, **kwargs
            )
            ...
            # add new correlation matrix with the present time step
            correlation_matrices[t_index] = pred_from_corr_matrices = correlation_matrices[
                t_index
            ].copy()
            ...
            for i, _ts in enumerate(timestamps_to_recalculate):
    ...
    def prune_state(self, predicted_state):
        ...
        if predicted_state.nstep > predicted_state.max_nstep != 0:
            ...
            predicted_state.multi_state_vector = predicted_state.multi_state_vector[:index]
            predicted_state.multi_covar = predicted_state.multi_covar[:index, :index]
            predicted_state.timestamps = predicted_state.timestamps[: predicted_state.max_nstep]
            predicted_state.correlation_matrices = predicted_state.correlation_matrices[
                : predicted_state.max_nstep
            ]
            return predicted_state
        else:
            return predicted_state
