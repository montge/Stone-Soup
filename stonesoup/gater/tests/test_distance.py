...
@pytest.mark.parametrize(
    ...
    [
        (  # Test 1
            {
                Detection(np.array([[2]])),
                Detection(np.array([[3]])),
                Detection(np.array([[6]])),
                Detection(np.array([[0]])),
                Detection(np.array([[-1]])),
                Detection(np.array([[-4]])),
            },
            1,
            3,
        ),
...
def test_distance(predictor, updater, detections, gate_threshold, num_gated):
    ...
    track = Track(
        [
            GaussianStateUpdate(
                np.array([[0]]),
                np.array([[1]]),
                SingleHypothesis(None, Detection(np.array([[0]]), metadata={"MMSI": 12345})),
                timestamp=timestamp,
            )
        ]
    )
    ...
    # gate threshold
    assert all(
        not hypothesis.measurement
        or measure(hypothesis.measurement_prediction, hypothesis.measurement) < gate_threshold
...
track = Track(
    [
        GaussianState(
            state_vector=[0, 0, 100, 0], timestamp=start_time, covar=np.diag([1.5, 0.5, 1.5, 0.5])
        )
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        # Perfect detection
        Detection(
            state_vector=[np.pi / 2, 100],
            timestamp=start_time,
            measurement_model=bearing_range_measurement_model,
        ),
        # Detection with slightly wrong range
        Detection(
            state_vector=[np.pi / 2, 109],
            timestamp=start_time,
            measurement_model=bearing_range_measurement_model,
        ),
        # Detection with slightly wrong bearing
        Detection(
            state_vector=[0.01 + np.pi / 2, 109],
            timestamp=start_time,
            measurement_model=bearing_range_measurement_model,
        ),
        # Perfect Linear Detection
        Detection(
            state_vector=[0, 100], timestamp=start_time, measurement_model=linear_measurement_model
        ),
    ],
...
def test_tracking_state_space_distance_gater_good_detections(
    detection,
    standard_distance_hypothesiser,
...
bearing_measurement_model = Cartesian2DToBearing(
    ndim_state=4, mapping=(0, 2), noise_covar=np.diag([1])
)
...
def test_tracking_state_space_distance_gater_irreversible_detections(
    detection,
    allow_non_reversible_detections,
    hypothesiser_with_tracking_state_space_distance_gater,
):
