...
def test_twodplotter():
    ...
    truths = {
        GroundTruthPath(
            states=[
                State(np.array([[1], [2]]), timestamp=timestamp1 + datetime.timedelta(seconds=i))
                for i in range(11)
            ]
        )
    }
    dets = {
        Detection(np.array([[1], [2]]), timestamp=timestamp1 + datetime.timedelta(seconds=i))
        for i in range(11)
    }
    ...
    metricD = plotter.plot_tracks_truth_detections(
        tracks=None, groundtruth_paths=truths, detections=dets
    )
    metrics.append(metricD)

    metricE = plotter.plot_tracks_truth_detections(
        tracks=tracks, groundtruth_paths=None, detections=dets
    )
