...
from ..uncertaintymetric import MeanofCovarianceNormsMetric, SumofCovarianceNormsMetric
...
def test_covariancenormsmetric_compute_covariancenorms(generator):
    ...
    track = Track(
        states=[
            GaussianState(
                state_vector=[[1], [2], [1], [2]], timestamp=time, covar=np.diag([i, i, i, i])
            )
            for i in range(5)
        ]
    )
    ...
    with pytest.raises(
        ValueError, match="All states must be from the same time to compute total uncertainty"
    ):
        generator.compute_covariancenorms(
            [
                GaussianState(
                    state_vector=[[1], [2], [1], [2]], timestamp=time, covar=np.diag([0, 0, 0, 0])
                ),
                GaussianState(
                    state_vector=[[1], [2], [1], [2]],
