...
@pytest.mark.parametrize(
    ...
    [
        (
            GaussianState(
                StateVector([0, 0, 0, 0]), CovarianceMatrix(np.diag([5.0, 10.0, 5.0, 10.0]))
            ),
            StateVectors([StateVector([[-10], [10]])]),
            1.0,
            [0, 2],
...
def test_compute_pcrb_single(
    ...
    metric = PCRBMetric._compute_pcrb_single(
        prior,
        transition_model,
        measurement_model,
        groundtruth,
