...
def test_gospametric_extractstates():
    ...
    generator = GOSPAMetric(c=10.0, p=1)
    # Test state extraction
    time_start = datetime.datetime.now()
    detections = [Detection(state_vector=np.array([[i]]), timestamp=time_start) for i in range(5)]
    tracks = {Track(states=[State(state_vector=[[i]], timestamp=time_start)]) for i in range(5)}
    truths = {
        GroundTruthPath(states=[GroundTruthState(state_vector=[[i]], timestamp=time_start)])
        for i in range(5)
    }
    ...
    assert set(track_states) == {t.states[0] for t in tracks}
    truth_states = generator.extract_states(truths)
    assert set(truth_states) == {t.states[0] for t in truths}
...
def test_gospametric_compute_assignments(num_states):
    ...
    track_obj = Track([State(state_vector=[[i]], timestamp=time_now) for i in range(num_states)])
    truth_obj = GroundTruthPath(
        [State(state_vector=[[i]], timestamp=time_now) for i in range(num_states)]
    )
    cost_matrix = generator.compute_cost_matrix(track_obj.states, truth_obj.states)
    neg_cost_matrix = -1.0 * cost_matrix
    meas_to_truth, truth_to_meas, opt_cost = generator.compute_assignments(neg_cost_matrix)

    assert opt_cost == 0.0
    assert np.array_equal(meas_to_truth, truth_to_meas)
    assert np.array_equal(meas_to_truth, np.array(list(range(num_states))))

    # Missing 1 track
    cost_matrix = generator.compute_cost_matrix(track_obj.states[:-1], truth_obj.states)
    neg_cost_matrix = -1.0 * cost_matrix
    meas_to_truth, truth_to_meas, opt_cost = generator.compute_assignments(neg_cost_matrix)
    ...
    assert np.array_equal(meas_to_truth, np.array(list(range(num_states - 1))))

    # Missing 1 truth
    cost_matrix = generator.compute_cost_matrix(track_obj.states, truth_obj.states[:-1])
    neg_cost_matrix = -1.0 * cost_matrix
    meas_to_truth, truth_to_meas, opt_cost = generator.compute_assignments(neg_cost_matrix)
    ...
    assert np.array_equal(meas_to_truth[:-1], np.array(list(range(num_states - 1))))
...
def test_gospametric_compute_gospa_metric():
    ...
    single_time_metric, _assignment_matrix = generator.compute_gospa_metric(
        track_obj.states, truth_obj.states
    )
...
def test_gospametric_computemetric(state_type):
    ...
    tracks = {
        Track(
            states=[
                state_type(state_vector=[[i + 0.5]], timestamp=time),
                state_type(state_vector=[[i + 1]], timestamp=time + datetime.timedelta(seconds=1)),
            ]
        )
        for i in range(5)
    }
    truths = {
        GroundTruthPath(
            states=[
                ...
                GroundTruthState(
                    state_vector=[[i]], timestamp=time + datetime.timedelta(seconds=1)
                ),
            ]
        )
        for i in range(5)
    ...
    assert main_metric.time_range.end_timestamp == time + datetime.timedelta(seconds=1)
    first_association = next(i for i in main_metric.value if i.timestamp == time)
    ...
    assert first_association.value["distance"] / 5.0 == 0.5
    second_association = next(
        i for i in main_metric.value if i.timestamp == time + datetime.timedelta(seconds=1)
    )
    ...
    assert second_association.value["distance"] / 5.0 == 1


def test_ospametric_extractstates():
    """Test OSPA metric extract states."""
    generator = OSPAMetric(c=10, p=1)
...
def test_ospametric_computecostmatrix():
    ...
    time = datetime.datetime.now()
    track = Track(states=[State(state_vector=[[i]], timestamp=time) for i in range(5)])
    truth = GroundTruthPath(states=[State(state_vector=[[i]], timestamp=time) for i in range(5)])
...
def test_ospametric_computeospadistance():
    ...
    truth = GroundTruthPath(
        states=[State(state_vector=[[i + 0.5]], timestamp=time) for i in range(5)]
    )
...
def test_ospametric_computemetric(p):
    ...
    tracks = {
        Track(
            states=[
                ...
                State(state_vector=[[i + 1.2]], timestamp=time + datetime.timedelta(seconds=1)),
            ]
        )
        for i in range(5)
    }
    truths = {
        GroundTruthPath(
            states=[
                GroundTruthState(state_vector=[[i]], timestamp=time),
                GroundTruthState(
                    state_vector=[[i + 1]], timestamp=time + datetime.timedelta(seconds=1)
                ),
            ]
        )
        for i in range(5)
...
def test_switching_gospametric_computemetric():
    ...
    switching_penalty = 3
    p = 2
    generator = GOSPAMetric(c=max_penalty, p=p, switching_penalty=switching_penalty)
    ...
    tracks = {
        Track(
            states=[
                State(state_vector=[[i]], timestamp=time)
                for i, time in zip([1, 2, 2], times, strict=False)
            ]
        ),
        Track(
            states=[
                State(state_vector=[[i]], timestamp=time)
                for i, time in zip([2, 1, 1], times, strict=False)
            ]
        ),
        Track(
            states=[
                State(state_vector=[[i]], timestamp=time)
                for i, time in zip([3, 100, 3], times, strict=False)
            ]
        ),
    }
    ...
    truths = {
        GroundTruthPath(
            states=[
                State(state_vector=[[i]], timestamp=time)
                for i, time in zip([1, 1, 1], times, strict=False)
            ]
        ),
        GroundTruthPath(
            states=[
                State(state_vector=[[i]], timestamp=time)
                for i, time in zip([2, 2, 2], times, strict=False)
            ]
        ),
        GroundTruthPath(
            states=[
                State(state_vector=[[i]], timestamp=time)
                for i, time in zip([3, 3, 3], times, strict=False)
            ]
        ),
    }
    ...
    assert (
        abs(
            second_association.value["distance"]
            - np.power(max_penalty**p + (2.5 ** (1 / p) * switching_penalty) ** p, 1.0 / p)
        )
        < 1e-9
    )
...
def test_gospametric_no_tracks():
    ...
    generator = GOSPAMetric(c=10, p=1)
...
def test_gospametric_speed():
    ...
    track_states = np.vstack((gt_states[:num_true_positives], 5 * gt_states[:num_false_positives]))
...
def test_switching_loss(associations, expected_losses):
    ...
    for association, expected_loss in zip(associations, expected_losses, strict=False):
