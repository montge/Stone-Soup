...
def test_basicmetrics():
    ...
    tracks = {
        Track(
            states=[
                State(np.array([[i], [j]]), timestamp=start_time + datetime.timedelta(seconds=i))
                for i in range(5)
            ]
        )
        for j in range(4)
    ...
    correct_metrics = {
        TimeRangeMetric(
            ...
            value=3,
            time_range=TimeRange(start=start_time, end=start_time + datetime.timedelta(seconds=4)),
            generator=generator,
        ),
        TimeRangeMetric(
    ...
    for metric_name in ["Number of targets", "Number of tracks", "Track-to-target ratio"]:
        calc_metric = next(i for i in correct_metrics if i.title == metric_name)
        meas_metric = metrics["basic_generator"].get(metric_name)
        assert calc_metric.value == meas_metric.value
        assert calc_metric.time_range.start_timestamp == meas_metric.time_range.start_timestamp
        assert calc_metric.time_range.end_timestamp == meas_metric.time_range.end_timestamp
