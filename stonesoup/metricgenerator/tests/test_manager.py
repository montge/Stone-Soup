...
class DummyMetricGenerator(MetricGenerator):
    ...
    def compute_metric(self, manager, *args, **kwargs):
        return Metric(title="Test metric1", value=25, generator=self)
...
def test_add_data_multimanager():
    ...
    tracks = [Track(states=State(np.array([[1]]), timestamp=datetime.datetime.now()))]
    truths = [GroundTruthPath(states=State(np.array([[2]]), timestamp=datetime.datetime.now()))]
    ...
    tracks2 = [Track(states=State(np.array([[21]]), timestamp=datetime.datetime.now()))]
    truths2 = [GroundTruthPath(states=State(np.array([[22]]), timestamp=datetime.datetime.now()))]
    ...
    manager.add_data(
        {"truths": truths2, "tracks": tracks, "tracks2": tracks2, "detections": dets2},
        overwrite=True,
    )
    ...
    manager.add_data({"truths": truths, "tracks": tracks, "detections": dets})
    manager.add_data(
        {"truths": truths + truths2, "tracks": tracks + tracks2, "detections": dets + dets2},
        overwrite=False,
    )
...
def test_associate_tracks_multimanager():
    generator = DummyMetricGenerator(
        generator_name="test_generator", tracks_key="tracks", truths_key="truths"
    )
...
def test_list_timestamps_multimanager():
    ...
    tracks = [Track(states=[State(np.array([[1]]), timestamp=timestamp1)])]
    truths = [GroundTruthPath(states=[State(np.array([[2]]), timestamp=timestamp2)])]
    manager.add_data({"truths": truths, "tracks": tracks})
...
def test_generate_metrics_multimanager():
    class DummyGenerator1(MetricGenerator):
        ...
        def compute_metric(self, *args, **kwargs):
            return Metric(title="Test metric2", value=25, generator=self)

    class DummyGenerator2(MetricGenerator):
        generator_name: str = Property()

        def compute_metric(self, *args, **kwargs):
            return Metric(title="Test metric3 at times", value=50, generator=self)
...
def test_generate_metrics_simplemanager():
    class DummyGenerator1(MetricGenerator):
        ...
        def compute_metric(self, *args, **kwargs):
            return Metric(title="Test metric4", value=30, generator=self)

    class DummyGenerator2(MetricGenerator):
        generator_name: str = Property()

        def compute_metric(self, *args, **kwargs):
            return Metric(title="Test metric5 at times", value=60, generator=self)
...
def test_association_caching_multimanager():
    ...
    new_truths = {
        GroundTruthPath(states=State(np.array([[3]]), timestamp=datetime.datetime.now()))
    }
...
def test_association_caching_key_specific():
    generator1 = DummyMetricGenerator(
        generator_name="test_generator1", tracks_key="tracks", truths_key="truths"
    )
    generator2 = DummyMetricGenerator(
        generator_name="test_generator2", tracks_key="tracks2", truths_key="truths2"
    )

    manager = MultiManager(associator=DummyAssociator(), generators=[generator1, generator2])
    tracks = {Track(states=State(np.array([[1]]), timestamp=datetime.datetime.now()))}
    truths = {GroundTruthPath(states=State(np.array([[2]]), timestamp=datetime.datetime.now()))}
    tracks2 = {Track(states=State(np.array([[3]]), timestamp=datetime.datetime.now()))}
    truths2 = {GroundTruthPath(states=State(np.array([[4]]), timestamp=datetime.datetime.now()))}

    manager.add_data({"truths": truths, "tracks": tracks, "truths2": truths2, "tracks2": tracks2})
    ...
    assert (
        manager._association_sets[(generator1.tracks_key, generator1.truths_key)]
