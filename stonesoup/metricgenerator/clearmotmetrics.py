...
class ClearMotMetrics(MetricGenerator):
    ...
    def compute_metric(self, manager: MultiManager, **kwargs) -> list[Metric]:
        ...
        motp = TimeRangeMetric(
            title="MOTP", value=motp_score, time_range=time_range, generator=self
        )
        mota = TimeRangeMetric(
            title="MOTA", value=mota_score, time_range=time_range, generator=self
        )
        return [motp, mota]
    ...
    def _compute_mota_and_motp(self, manager: MultiManager) -> tuple[float, float]:
        ...
        motp = (
            (error_sum / num_associated_truth_timestamps)
    ...
    def _compute_sum_of_distances_at_timestep(
        self,
    ...
    def _create_matches_at_time_lookup(
    ...
    def _compute_number_of_miss_matches_from_match_sets(
        self, matches_prev: MatchSetAtTimestamp, matches_current: MatchSetAtTimestamp
    ) -> int:
...
def _create_state_from_time_and_id_lookup(
    tracks_set: set[Track | GroundTruthPath],
...
def check_matches_for_metric_calculation(
    ...
    for t, matches in matches_by_timestamp.items():
        ...
        if len(truth_ids) > len(set(truth_ids)):
            raise AssociationSetNotValid(
                f"Multiple tracks are assigned with "
        ...
        if len(track_ids) > len(set(track_ids)):
            raise AssociationSetNotValid(
                f"A single track is assigned with "
