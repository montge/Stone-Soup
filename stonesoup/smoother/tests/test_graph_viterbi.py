...
def simple_track(measurement_model):
    ...
    positions = [(0.1, 0.1), (1.1, 0.1), (2.0, 0.0), (3.1, 0.0), (4.0, 0.0)]

    for (x, y), timestamp in zip(positions, times, strict=False):
...
def test_graph_viterbi_instantiation_dict(transition_model, measurement_model, simple_graph_dict):
...
def test_graph_viterbi_instantiation_with_params(
    transition_model, measurement_model, simple_graph_dict
):
...
def test_graph_viterbi_parse_graph_dict(transition_model, measurement_model, simple_graph_dict):
...
def test_graph_viterbi_find_nearest_node(transition_model, measurement_model, simple_graph_dict):
...
def test_graph_viterbi_get_neighbors(transition_model, measurement_model, simple_graph_dict):
...
def test_graph_viterbi_empty_track(transition_model, measurement_model, simple_graph_dict):
...
def test_graph_viterbi_smooth_basic(
    transition_model, measurement_model, linear_graph_dict, simple_track
):
...
def test_graph_viterbi_smooth_constrained_to_graph(
    transition_model, measurement_model, linear_graph_dict
):
    ...
    for i, timestamp in enumerate(times):
        ...
        covar = np.eye(4) * 0.1
        detection = Detection(np.array([[float(i) + 0.3], [0.5]]), timestamp=timestamp)
        pred = GaussianStatePrediction(state_vec, covar, timestamp=timestamp)
        hypothesis = SingleHypothesis(pred, detection)
        state = GaussianStateUpdate(state_vec, covar, hypothesis, timestamp=timestamp)
        track.append(state)
...
def test_graph_viterbi_single_state_track(transition_model, measurement_model, simple_graph_dict):
...
def test_graph_viterbi_log_transition_probability(
    transition_model, measurement_model, simple_graph_dict
):
...
def test_graph_viterbi_log_observation_likelihood(
    transition_model, measurement_model, simple_graph_dict
):
...
def test_graph_viterbi_off_road_penalty(transition_model, measurement_model, linear_graph_dict):
...
def test_graph_viterbi_with_predictions(transition_model, measurement_model, linear_graph_dict):
...
def test_graph_viterbi_preserves_state_type(
    transition_model, measurement_model, simple_graph_dict
):
...
def test_graph_viterbi_node_position_retrieval(
    transition_model, measurement_model, simple_graph_dict
):
...
def test_graph_viterbi_square_graph_path(transition_model, measurement_model, simple_graph_dict):
...
def test_graph_viterbi_numerical_stability(transition_model, measurement_model, linear_graph_dict):
...
def test_graph_viterbi_with_state_measurement(
    transition_model, measurement_model, simple_graph_dict
):
...
def test_graph_viterbi_networkx_graph(transition_model, measurement_model):
    ...
    smoother = GraphViterbiSmoother(
        graph=G, transition_model=transition_model, measurement_model=measurement_model
    )
