...
try:
    ...
    warnings.warn(f"Failed to import 'rtree': {err!r}", stacklevel=2)
    rtree = None
...
class DetectionKDTreeMixIn(Base):
    ...
    max_distance: float = Property(
        default=np.inf, doc="Max distance to return points. Default `inf`"
    )
    ...
    def generate_hypotheses(self, tracks, detections, timestamp, **kwargs):
        ...
        tree = KDTree(np.vstack([detection.state_vector[:, 0] for detection in detections_list]))
        ...
        for track in tracks:
            ...
            if self.number_of_neighbours is None:
                indexes = tree.query_ball_point(meas_pred_state_vector.ravel(), r=max_distance)
            else:
...
class TPRTreeMixIn(Base):
    ...
    def _track_tree_coordinates(self, track):
        ...
        state_delta = 3 * np.sqrt(np.diag(track.covar)[self.pos_mapping,].reshape(-1, 1))
        vel_vector = track.mean[self.vel_mapping, :]
        vel_delta = 3 * np.sqrt(np.diag(track.covar)[self.vel_mapping,].reshape(-1, 1))
    ...
    def generate_hypotheses(self, tracks, detections, timestamp, **kwargs):
        ...
        for track in sorted_tracks:
            if track not in self._tree:  # track not in tree, so insert it
                ...
                coords = (
                    *self._coords[track][:-1],
                    (
                        self._coords[track][-1] - 1e-06,
        ...
        for detection in sorted(detections, key=attrgetter("timestamp")):
            ...
            if isinstance(model, LinearModel):
                ...
                state_meas = (inv_model_matrix @ detection.state_vector)[self.pos_mapping, :]
            else:
                state_meas = model.inverse_function(detection, **kwargs)[self.pos_mapping, :]
