...
def tracks():
    ...
    tracks = [
        Track(
            states=[
                State(
                    state_vector=[[i], [i]], timestamp=start_time + datetime.timedelta(seconds=i)
                )
                for i in range(10)
            ]
        )
    ...
    tracks.append(
        Track(
            states=[State(state_vector=[[20], [20]], timestamp=start_time)]
            + [
                State(
                    state_vector=[[i + 2], [i + 2]],
...
def test_euclidiantracktotrack(tracks):
    ...
    association_set_1 = complete_associator.associate_tracks(
    ...
    association_set_2 = positional_associator.associate_tracks(
        {tracks[0], tracks[2]}, {tracks[1], tracks[3], tracks[4]}
    )
    ...
    assoc1 = next(iter(association_set_1.associations))
    assert set(assoc1.objects) == {tracks[0], tracks[1]}
    assert assoc1.time_range.start_timestamp == start_time + datetime.timedelta(seconds=1)
    assert assoc1.time_range.end_timestamp == start_time + datetime.timedelta(seconds=6)

    assert len(association_set_2.associations) == 2
    assoc20 = next(iter(association_set_2.associations))
    assoc21 = list(association_set_2.associations)[1]
    assert set(assoc20.objects) == {tracks[0], tracks[4]} or set(assoc21.objects) == {
        tracks[0],
        tracks[4],
    }

    assert len(association_set_3.associations) == 2
    assoc30 = next(iter(association_set_3.associations))
    assoc31 = list(association_set_3.associations)[1]
    assert set(assoc30.objects) == {tracks[0], tracks[4]} or set(assoc31.objects) == {
        tracks[0],
        tracks[4],
    }

    assert len(association_set_4) == 1
    assoc4 = next(iter(association_set_4))
    assert assoc4.time_range.start_timestamp == start_time + datetime.timedelta(seconds=1)
    assert assoc4.time_range.end_timestamp == start_time + datetime.timedelta(seconds=7)

    assert len(association_set_one2one) == 1
    assoc5 = next(iter(association_set_one2one))
    # assoc5 should be equal to assoc1
    assert set(assoc5.objects) == {tracks[0], tracks[1]}
    assert assoc5.time_range.start_timestamp == start_time + datetime.timedelta(seconds=1)
    assert assoc5.time_range.end_timestamp == start_time + datetime.timedelta(seconds=6)


def test_euclidiantracktotruth(tracks):
    associator = TrackToTruth(
        association_threshold=10, consec_pairs_confirm=3, consec_misses_end=2
    )
    start_time = datetime.datetime(2019, 1, 1, 14, 0, 0)
    ...
    assoc = next(iter(association_set.associations))
    assert set(assoc.objects) == {tracks[0], tracks[1]}
    assert assoc.time_range.start_timestamp == start_time + datetime.timedelta(seconds=1)
    assert assoc.time_range.end_timestamp == start_time + datetime.timedelta(seconds=6)
...
def test_empty_track_to_truth(tracks):
    ...
    associated_objects = {obj for association in association_set for obj in association.objects}
...
def test_trackidbased():
    ...
    tracksA = [
        Track(
            states=[
                State(state_vector=[[i]], timestamp=start_time),
                State(state_vector=[[i]], timestamp=start_time + datetime.timedelta(seconds=1)),
                State(state_vector=[[i]], timestamp=start_time + datetime.timedelta(seconds=2)),
            ],
            id=f"id{i}",
        )
        for i in range(0, 5)
    ...
    tracksB = [Track(states=[State(state_vector=[[i]])], id=f"id{i}") for i in range(0, 5)]

    tracksC = [
        Track(
            states=[
                State(state_vector=[[i]], timestamp=start_time + datetime.timedelta(seconds=4)),
                State(state_vector=[[i]], timestamp=start_time + datetime.timedelta(seconds=5)),
            ],
            id=f"id{i}",
        )
        for i in range(0, 5)
    ...
    truths = [
        GroundTruthPath(
            states=[
                GroundTruthState(
                    state_vector=[[i + 0.5]], timestamp=start_time + datetime.timedelta(seconds=1)
                ),
                GroundTruthState(
                    state_vector=[[i + 0.5]], timestamp=start_time + datetime.timedelta(seconds=2)
                ),
                GroundTruthState(
                    state_vector=[[i + 0.5]], timestamp=start_time + datetime.timedelta(seconds=3)
                ),
            ],
            id=f"id{i}",
        )
    ...
    assocA = next(iter(association_setA.associations))
