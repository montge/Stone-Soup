...
from ...types.particle import MultiModelParticle, Particle
from ...types.prediction import ParticleMeasurementPrediction, ParticleStatePrediction
from ...types.state import MultiModelParticleState, ParticleState
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (None, False, None),  # transition_model  # model_flag  # constraint_function
        (
            CombinedLinearGaussianTransitionModel([ConstantVelocity(0.05)]),  # transition_model
            False,  # model_flag
    ...
    ids=[
        "with_transition_model_init",
        "without_transition_model_init",
        "no_transition_model",
...
def test_regulariser(transition_model, model_flag, constraint_func):
    ...
    if transition_model is not None:
        new_state_vector = transition_model.function(
            particles, noise=True, time_interval=datetime.timedelta(seconds=1)
        )
    else:
        new_state_vector = particles.state_vector

    prediction = ParticleStatePrediction(
        new_state_vector, timestamp=timestamp, transition_model=transition_model
    )

    measurement_model = LinearGaussian(ndim_state=2, mapping=(0, 1), noise_covar=np.eye(2))
    measurement = Detection(
        state_vector=np.array([[5], [7]]), timestamp=timestamp, measurement_model=measurement_model
    )
    hypothesis = SingleHypothesis(
        prediction=prediction, measurement=measurement, measurement_prediction=None
    )

    state_update = Update.from_state(
        state=prediction,
    ...
    state_update.weight = np.array(
        [1 / 6, 5 / 48, 5 / 48, 5 / 48, 5 / 48, 5 / 48, 5 / 48, 5 / 48, 5 / 48]
    )
    ...
    if model_flag:
        ...
        regulariser = MCMCRegulariser(
            transition_model=transition_model, constraint_func=constraint_func
        )
    ...
    with pytest.raises(TypeError) as e:
        new_particles = regulariser.regularise(particles.particle_list, state_update)
    assert "Only ParticleState type is supported!" in str(e.value)
    with pytest.raises(Exception) as e:
        new_particles = regulariser.regularise(particles, state_update.particle_list)
    assert "Only ParticleState type is supported!" in str(e.value)
...
def test_no_measurement():
    ...
    timestamp = datetime.datetime.now()
    prediction = ParticleStatePrediction(
        None, particle_list=particles.particle_list, timestamp=timestamp
    )
    meas_pred = ParticleMeasurementPrediction(None, particle_list=particles, timestamp=timestamp)
    state_update = ParticleStateUpdate(
        None,
        SingleHypothesis(
            prediction=prediction, measurement=None, measurement_prediction=meas_pred
        ),
...
def test_multi_model_regulariser(
    dynamic_model_list, position_mappings, transition_matrix
):  # noqa: F811

    # Initialise particles
    particle1 = MultiModelParticle(
        state_vector=[1, 1, 0.5, 1, 1, 0.5], weight=1 / 3000, dynamic_model=0
    )
    particle2 = MultiModelParticle(
        state_vector=[1, 1, 0.5, 1, 1, 0.5], weight=1 / 3000, dynamic_model=1
    )
    particle3 = MultiModelParticle(
        state_vector=[1, 1, 0.5, 1, 1, 0.5], weight=1 / 3000, dynamic_model=2
    )

    particles = [particle1, particle2, particle3] * 1000
    timestamp = datetime.datetime.now()

    particle_state = MultiModelParticleState(None, particle_list=particles, timestamp=timestamp)

    predictor = MultiModelPredictor(dynamic_model_list, transition_matrix, position_mappings)
    ...
    detection = Detection([[6.0, 7.0]], timestamp, measurement_model)

    update = updater.update(hypothesis=SingleHypothesis(prediction, detection))

    regulariser = MultiModelMCMCRegulariser(dynamic_model_list, position_mappings)
