...
def test_pda(predictor, updater, normalise):
    ...
    mulltihypothesis = hypothesiser.hypothesise(track, detections, timestamp)
    ...
    assert all(
        hypothesis.probability >= 0 and isinstance(hypothesis.probability, Probability)
    ...
    assert any(
        isinstance(hypothesis.measurement, MissedDetection) for hypothesis in mulltihypothesis
    )
    ...
    hypothesiser = PDAHypothesiser(
        predictor, updater, prob_detect=0.9, prob_gate=0.99, normalise=normalise
    )
...
def test_invalid_pda_arguments(predictor, updater):
    with pytest.raises(ValueError):
        PDAHypothesiser(predictor, updater, prob_detect=0.9, prob_gate=0.99, include_all=True)
