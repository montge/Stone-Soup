...
def hypothesiser(updater, predictor, request):
    ...
    hypothesiser = DistanceHypothesiser(predictor, updater, measure=measure, missed_distance=20)
    kwargs = {"hypothesiser": hypothesiser, "order_by_detection": True}
...
def test_gm_ordered_by_measurement(hypothesiser):
    ...
    gaussian_mixture = GaussianMixture(
        [
            WeightedGaussianState(np.array([[0.3]]), np.array([[1]]), timestamp, 0.4),
            WeightedGaussianState(np.array([[5]]), np.array([[0.5]]), timestamp, 0.3),
        ]
    )
    detection1 = Detection(np.array([[1]]), timestamp=timestamp + datetime.timedelta(seconds=1))
    detection2 = Detection(np.array([[6.2]]), timestamp=timestamp + datetime.timedelta(seconds=1))
    detections = {detection1, detection2}

    hypotheses = hypothesiser.hypothesise(gaussian_mixture, detections, timestamp)

    # There are 4 hypotheses - 2 each associated with detection1/detection2
    assert all(isinstance(multi_hyp, MultipleHypothesis) for multi_hyp in hypotheses)
    assert all(isinstance(hyp, SingleHypothesis) for multi_hyp in hypotheses for hyp in multi_hyp)
    ...
    assert all(hyp.distance >= 0 for multi_hyp in hypotheses for hyp in multi_hyp)
    ...
    assert len({hypothesis.measurement for hypothesis in hypotheses[0]}) == 1
    assert len({hypothesis.measurement for hypothesis in hypotheses[1]}) == 1
