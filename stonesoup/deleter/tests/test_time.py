...
from ..time import UpdateTimeDeleter, UpdateTimeStepsDeleter
...
def test_update_time_steps_deleter():
    ...
    track = Track(
        [
            ...
            StatePrediction([[0]], timestamp=datetime.datetime(2018, 1, 1, 14, 10)),
            StatePrediction([[0]], timestamp=datetime.datetime(2018, 1, 1, 14, 20)),
        ]
    )
    ...
    track.append(StatePrediction([[0]], timestamp=datetime.datetime(2018, 1, 1, 14, 30)))
    ...
    track.append(
        StateUpdate(
            [[0]], SingleHypothesis(None, None), timestamp=datetime.datetime(2018, 1, 1, 14, 30)
        )
    )
    tracks2delete = deleter.delete_tracks({track})
    assert tracks2delete
...
def test_update_time_deleter():
    ...
    tracks2delete = deleter.delete_tracks({track}, timestamp=datetime.datetime(2018, 1, 1, 14, 15))
    assert not tracks2delete

    # Last update outside custom time
    tracks2delete = deleter.delete_tracks({track}, timestamp=datetime.datetime(2018, 1, 1, 14, 25))
    assert tracks2delete
