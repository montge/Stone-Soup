...
from .base import GaussianInitiator, Initiator, ParticleInitiator
...
class SinglePointInitiator(GaussianInitiator):
    ...
    updater: Updater = Property(
        default=None, doc="Updater to use. Defaults to `None` where EKF will be used."
    )
    ...
    def initiate(self, detections, timestamp, **kwargs):
        ...
        for detection in detections:
            ...
            track_state = self.updater.update(
                SingleHypothesis(self.prior_state, detection, measurement_prediction)
            )
...
class SimpleMeasurementInitiator(GaussianInitiator):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.diag_load < 0:
            raise ValueError("diag_load value can't be less than 0.0")
...
class MultiMeasurementInitiator(GaussianInitiator):
    ...
    skip_non_reversible: bool = Property(
        default=False,
        doc="Skip measurements that do not have a reversible measurement model. "
    ...
    def initiate(self, detections, timestamp, **kwargs):
        ...
        if self.holding_tracks:
            ...
            for track, hypothesis in associations.items():
                ...
                if (
                    sum(1 for state in track if not self.updates_only or isinstance(state, Update))
                    >= self.min_points
                ):
...
class GaussianParticleInitiator(ParticleInitiator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            samples = multivariate_normal.rvs(
                self.initiator.prior_state.state_vector.ravel(),
                self.initiator.prior_state.covar,
                size=self.number_particles,
            )
        except AttributeError:
            raise AttributeError("No prior state")
        particles = [Particle(sample.reshape(-1, 1), weight=self.weight) for sample in samples]
    ...
    def initiate(self, detections, timestamp, **kwargs):
        ...
        for track in tracks:
            samples = multivariate_normal.rvs(
                track.state_vector.ravel(), track.covar, size=self.number_particles
            )
            particles = [Particle(sample.reshape(-1, 1), weight=self.weight) for sample in samples]
...
class GaussianMixtureInitiator(GaussianInitiator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.prior_state = GaussianMixture(
            [
                TaggedWeightedGaussianState(
                    state_vector=state, covar=covar, weight=Probability(1), tag=[]
                )
            ]
        )
        ...
        for track in tracks:
            for n, state in enumerate(track):
                ...
                track[n] = GaussianMixtureUpdate(
                    hypothesis=getattr(state, "hypothesis", None), components=mixture
                )
...
class ASDGaussianInitiator(GaussianInitiator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.prior_state = ASDGaussianState(
            multi_state_vector=state, timestamps=None, max_nstep=self.max_nstep, multi_covar=covar
        )
...
class EnsembleInitiator(GaussianInitiator):
    ...
    ensemble_size: int = Property(
        default=100, doc="Integer to determine the size of the Gaussian Ensemble State."
    )
    ...
    def initiate(self, detections, timestamp, **kwargs):
        ...
        for track in tracks:
            gaussian_state = GaussianState(track.state_vector, track.covar, track.timestamp)

            track[-1] = EnsembleStateUpdate.from_gaussian_state(
                gaussian_state, self.ensemble_size, hypothesis=track.hypothesis
            )
