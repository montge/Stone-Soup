...
from ...types.detection import CategoricalDetection, CompositeDetection, Detection
...
from ..simple import GaussianParticleInitiator, SinglePointInitiator
...
def composite_measurements(num_sub_states, ndim_states, timestamp):
    ...
    if num_sub_states > 1:
        ...
        sub_states = (
            measurement2.sub_states[:missed_index] + measurement2.sub_states[missed_index + 1 :]
        )
...
def initiators_measurement(num_sub_states, timestamp):
    ...
    sub_priors = [
        ...
        create_state(False, False, ndim_states[4], None),
    ]  # Categorical prior
...
def test_simple_composite(num_initiators):
    ...
    # Test prior
    for actual_sub_state, sub_initiator in zip(
        initiator.prior_state, initiator.sub_initiators, strict=False
    ):
