...
@pytest.fixture(params=[SystematicResampler(), None], ids=["systematic_resampler", "no_resampler"])
    ...
    class DummyUpdater(ParticleUpdater):
        ...
        def update(self, hypothesis, **kwargs):
            ...
            return Update.from_state(
                state=prediction, hypothesis=hypothesis, timestamp=prediction.timestamp
            )
        ...
        def get_log_weights_per_hypothesis(self, hypothesis):
            ...
            for i, hyp in enumerate(hypothesis):
                if not hyp:
                    ...
                    log_weights_per_hyp[:, 0] = np.log(0.9 / num_samples)
                    continue
                if isinstance(hyp.measurement, TrueDetection):
                    log_weights_per_hyp[:, i] = np.log(0.8 / num_samples)
                else:
                    log_weights_per_hyp[:, i] = np.log(0.1 / num_samples)
...
@pytest.mark.parametrize(
    ...
    [(100, 150, 0.7, SystematicResampler()), (100, 200, 0.7, None), (100, 200, 0.9, None)],
)
def test_smc_phd_initiator(
    num_particles, num_track_samples, threshold, resampler, dummy_predictor, dummy_updater
):
    ...
    prior_sv = StateVectors([list(range(num_particles))])
    prior = ParticleState(
        state_vector=prior_sv,
