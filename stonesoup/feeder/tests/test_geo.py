...
from ..geo import LLAtoENUConverter, LLAtoNEDConverter, LongLatToUTMConverter
...
def test_lla_reference_converter(reader, converter_class, reverse_func):
    ...
    for i, (_time, detections) in zip(range(-3, 4), converter, strict=False):
        detection = detections.pop()

        assert pytest.approx((50, i, 5000 + i * 10), abs=1e-2, rel=1e-3) == reverse_func(
            *detection.state_vector, 50, 0, 5000
        )
...
def test_utm_converter(reader):
    ...
    for long, (_time, detections) in zip(range(-3, 4), converter, strict=False):
        detection = detections.pop()
