...
t2 = Track(
    [
        GaussianState([1, 1, 1, 1], np.diag([2, 2, 2, 2]), timestamp=1),
        GaussianState([2, 1, 2, 1], np.diag([2, 2, 2, 2]), timestamp=2),
    ]
)
t3 = Track(
    [
        GaussianState([1, 1], np.diag([2, 2]), timestamp=0),
...
@pytest.mark.parametrize("tracks", [([t1]), ([t1, t2]), ([t2, t3]), ([t1, t2, t3, t4])])
def test_Track2GaussianDetectionFeeder(tracks):
    ...
    assert np.all(
        [
            len(detection.state_vector) == len(track.state_vector)
            for track in tracks
            for detection in detections
            if detection.metadata["track_id"] == track.id
        ]
    ...
    assert np.all(
        [
            len(detection.state_vector) == detection.measurement_model.ndim
            for detection in detections
        ]
    )
    ...
    for track in tracks:
        detection = next(
            iter(
                detection for detection in detections if detection.metadata["track_id"] == track.id
            )
        )
