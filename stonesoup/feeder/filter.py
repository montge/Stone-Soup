    ...
    * If :class:`~.GroundTruthPath` type is extended to have a metadata attribute, this class
    ...
    def data_gen(self):
        for time, detections in self.reader:
            ...
            sorted_detections = sorted(detections, key=attrgetter("timestamp"), reverse=True)
            meta_values = set()
    ...
    def data_gen(self):
        for time, detections in self.reader:
            ...
            for detection in detections:
                ...
                if (value is None and self.keep_unmatched) or (
                    value is not None and self.operator(value)
                ):
    ...
    * Finally, for simplicity purposes, the bounding box is not allowed to
    ...
    def data_gen(self):
        ...
        for time, states in self.reader:
            outlier_data = set()
            for state in states:
                if self.apply_measurement_model_inverse and getattr(
                    state, "measurement_model", None
                ):
