...
def test_simple_sensor_seed():
    ...
    for m1, m2 in zip(measurements1, measurements2, strict=False):
        assert m1.state_vector[0] != m2.state_vector[0]
    for m3, m4 in zip(measurements3, measurements4, strict=False):
...
def radar_platform_target():
    ...
    radar = RadarElevationBearingRangeRate(
        ndim_state=6,
    ...
    platform = FixedPlatform(
        states=platform_1_state, position_mapping=pos_mapping, sensors=[radar]
    )
...
def test_sensor_assignment(radar_platform_target):
    radar_1, platform, _target_state = radar_platform_target
...
def test_informative():
    dummy_truth1 = GroundTruthPath(
        [
            GroundTruthState([1, 1, 1, 1]),
            GroundTruthState([2, 1, 2, 1]),
            GroundTruthState([3, 1, 3, 1]),
        ]
    )
    dummy_truth2 = GroundTruthPath(
        [
            GroundTruthState([1, 0, -1, -1]),
    ...
    test_radar = RadarBearingRange(
        ndim_state=4,
        position_mapping=(0, 2),
        noise_covar=np.diag([np.radians(1) ** 2, 5**2]),
        movement_controller=FixedMovable(
            states=[position], orientation=orientation, position_mapping=(0, 2)
        ),
    )
    test_sensor = SensorSuite(attributes_inform=["position", "orientation"], sensors=[test_radar])
    ...
    test_radar2 = RadarElevationBearingRange(
        ndim_state=4,
        position_mapping=(0, 2, 3),
        noise_covar=np.diag([np.radians(10) ** 2, np.radians(2) ** 2, 0.1**2]),
        movement_controller=FixedMovable(
            states=[position2], orientation=orientation2, position_mapping=(0, 2, 3)
        ),
    )

    test_sensor = SensorSuite(
        attributes_inform=["position", "orientation"], sensors=[test_radar, test_radar2]
    )
