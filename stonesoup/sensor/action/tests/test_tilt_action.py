...
from ..tilt_action import ChangeTiltAction, TiltActionsGenerator
...
def test_tilt_action(initial_tilt_deg):
    ...
    actionable = DummyTiltActionable(
        StateVector([initial_tilt]), start, 2 / 3
    )  # 4 degrees per second

    generator = TiltActionsGenerator(
        actionable, attribute="tilt_centre", start_time=start, end_time=end, rpm=actionable.rpm
    )
    ...
    for angle in np.linspace(0, np.radians(60), 10):
        ...
        if tilt1 == actionable.max_tilt:
            new_tilt_time = (
                (actionable.max_tilt - actionable.tilt_centre[0, 0])
                / np.radians(60)
                * (end - start)
            )
        ...
        if tilt2 == actionable.min_tilt:
            new_tilt_time = (
                (actionable.tilt_centre[0, 0] - actionable.min_tilt)
                / np.radians(60)
                * (end - start)
            )
            new_tilt_end = start + new_tilt_time
    ...
    if (
        actionable.min_tilt + np.radians(60)
        ...
        actionable.min_tilt + np.radians(30)
        <= actionable.tilt_centre[0, 0]
    ...
    for action, target_tilt in zip(actions, target_tilts, strict=False):
